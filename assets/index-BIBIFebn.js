...
`;function je(){if(document.getElementById(be))return;const n=document.createElement("style");n.id=be,n.textContent=Qe,document.head.appendChild(n)}function Ne(n){je();const a=document.createElement("div");a.className="rml",a.innerHTML=`
  ...
  `;const o=n.querySelector("[data-main]"),b=n.querySelector("[data-nav]"),v=n.querySelector("[data-lang]"),O=n.querySelector("[data-strand-back]");let G=t;function $(C){G=C,b.innerHTML=Ue.map(F=>`<button type="button" class="${C===F?"active":""}" data-sec="${F}">${s(Ke[F])}</button>`).join(""),b.querySelectorAll("button").forEach(F=>{F.addEventListener("click",()=>r(F.getAttribute("data-sec")))})}function _(){v.innerHTML=`
    ...
    <section class="panel panel--worksheets">
      ...
      <div class="worksheet-output" data-ws-out><p class="lead">${n("worksheets.empty")}</p></div>
        ...
        <p class="q-exp q-summary"><em>${ne(u.exp)}</em></p>`);const w=E?"correct":R?"incorrect":"",X=u.choices.map((k,Q)=>`<li>
          <label class="choice-radio">
            ...
            <strong>${h[Q]}.</strong> ${ne(k)}
          </label>
        </li>`).join("");return`<div class="question-row ${w}" data-ws-row="${i}">
      <div class="q-header"><h4>Q${g} ${T}</h4>${E?'<span class="result-icon">✓</span>':R?'<span class="result-icon">✗</span>':""}</div>
      <p>${ne(u.q)}</p>
      <ol class="choice-list choice-list--radio">${X}</ol>
      ${Y}
    </div>`}function y(){o.items.length&&(G.innerHTML=o.items.map((e,i)=>f(e,i,o.tab)).join(""),W())}function W(){o.tab==="practice"&&G.querySelectorAll("[data-ws-choice]").forEach(e=>{e.addEventListener("change",()=>{const i=Number(e.getAttribute("data-ws-q"));if(o.resolved[i])return;const d=Number(e.getAttribute("data-ws-choice")),u=o.items[i][U()]||o.items[i].en;o.userAnswers[i]=d,d===u.a?o.resolved[i]=!0:(o.wrongAttempts[i]=(o.wrongAttempts[i]||0)+1,o.wrongAttempts[i]>=2&&(o.resolved[i]=!0)),K(),B()&&H(),y()})})}function H(){const e=o.items.length;let i=0,d=0;const u={};o.items.forEach((S,A)=>{const L=S[U()]||S.en;if(o.userAnswers[A]===L.a)i+=1,(o.wrongAttempts[A]||0)===0&&(d+=1);else if(o.resolved[A]){const R=S.section||S.topic;u[R]=(u[R]||0)+1}});const h=e?Math.round(i/e*100):0;let g=t("worksheets.revisionFair");h>=90?g=t("worksheets.revisionExcellent"):h>=70?g=t("worksheets.revisionGood"):h<50&&(g=t("worksheets.revisionLow"));const T=Object.entries(u).sort((S,A)=>A[1]-S[1]).map(([S,A])=>`<li>${ne(S)} (${A})</li>`).join("");F.innerHTML=`
      ...
      ${T?`<p>${t("worksheets.weakSections")}:</p><ul>${T}</ul>`:""}`,F.hidden=!1}function p(e){const i=U(),d=o.items.map((u,h)=>{const g=u[i]||u.en,T=g.choices.map((A,L)=>`${fe[L]}. ${A}`).join("<br/>"),S=e?`<p><b>${t("worksheets.answer")}:</b> ${fe[g.a]}<br/><i>${g.exp}</i></p>`:"";return`<div style="page-break-inside:avoid;margin-bottom:16px"><b>Q${h+1}.</b> ${g.q}<br/>${T}${S}</div>`}).join("");return`<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word">
...
`;function rt(){if(document.getElementById(ye))return;const n=document.createElement("style");n.id=ye,n.textContent=it,document.head.appendChild(n)}function ot(n){rt();const a=document.createElement("div");a.className="tl-wrap",a.innerHTML=`
  ...
  `;const t={liquidType:"mercury",thermometerType:"liquid",bulbVolume:200,wallThickness:.5,capillaryBore:.3,liquidL0:3,liquidL100:13,resistanceR0:5,resistanceR100:6.2,thermistorR25:10,thermistorBeta:3500,bathTemp:25,thermometerTemp:25,bubbles:[],iceCubes:[],heatWaves:[],currentLength:5.5,currentResistance:5.3,currentThermistorR:10,lastTimestamp:0},r=460,c=340,o=80,b=680,v=480;function O(){const e={left:72,top:48,right:24,bottom:52};return{gx:e.left,gy:e.top,gw:b-e.left-e.right,gh:v-e.top-e.bottom,tickFont:`${Math.round(b*.02)}px Arial`,axisFont:`bold ${Math.round(b*.022)}px Arial`,dotR:5,yLabelX:e.left-34,xLabelY:v-e.bottom+18}}const G=a.querySelector("#tl-thermometerCanvas"),$=G.getContext("2d"),_=a.querySelector("#tl-graphCanvas"),I=_.getContext("2d"),N=window.devicePixelRatio||1;G.width=r*N,G.height=c*N,$.scale(N,N),_.width=b*N,_.height=v*N,I.scale(N,N);function C(){t.iceCubes=[];for(let e=0;e<4;e++)t.iceCubes.push({x:60+Math.random()*120,y:200+Math.random()*15,size:10+Math.random()*8,angle:Math.random()*Math.PI,speedX:(Math.random()-.5)*.15,speedY:(Math.random()-.5)*.15});t.bubbles=[];for(let e=0;e<15;e++)t.bubbles.push({x:60+Math.random()*120,y:190+Math.random()*70,r:1+Math.random()*3,speedY:.6+Math.random()*1.2,phase:Math.random()*Math.PI*2,wobbleSpeed:.08+Math.random()*.08});t.heatWaves=[];for(let e=0;e<6;e++)t.heatWaves.push({x:65+Math.random()*110,y:190+Math.random()*60,length:12+Math.random()*15,speedY:.4+Math.random()*.5,opacity:.1+Math.random()*.25})}function F(){if(t.thermometerType==="resistance"||t.thermometerType==="thermistor")return .35;const e=t.liquidType==="mercury"?1:8,i=.2+t.wallThickness*1.5,d=.4+t.bulbVolume*.003,u=1.35-t.capillaryBore*.45;return Math.max(.1,i*d*u*e*.15)}function z(e){t.bathTemp<=8&&t.iceCubes.forEach(i=>{i.x+=i.speedX,i.y+=i.speedY,(i.x<65||i.x>175)&&(i.speedX*=-1),(i.y<195||i.y>210)&&(i.speedY*=-1)}),t.bathTemp>35&&t.heatWaves.forEach(i=>{i.y-=i.speedY*(1+t.bathTemp/100),i.y<185&&(i.y=265,i.x=65+Math.random()*110)}),t.bathTemp>=80&&t.bubbles.forEach(i=>{i.y-=i.speedY*(1+(t.bathTemp-80)/40),i.phase+=i.wobbleSpeed,i.x+=Math.sin(i.phase)*.2,i.y<185&&(i.y=260+Math.random()*10,i.x=60+Math.random()*120)})}function P(e){let T=59,S=130,A=246;if(t.bathTemp<25){const w=t.bathTemp/25;T=Math.round(180-w*121),S=Math.round(210-w*80),A=Math.round(250-w*4)}else{const w=Math.min(1,(t.bathTemp-25)/125);T=Math.round(59+w*180),S=Math.round(130-w*100),A=Math.round(246-w*190)}if(t.bathTemp>40){const w=Math.min(1,(t.bathTemp-40)/100),X=Date.now()*.02;e.fillStyle="rgba(249, 115, 22, 0.85)",e.beginPath(),e.moveTo(55+130/2-20,284);for(let D=-20;D<=20;D+=8){const k=w*(10+Math.sin(D*.3+X)*4);e.lineTo(55+130/2+D,284-k)}e.lineTo(55+130/2+20,284),e.closePath(),e.fill()}e.strokeStyle="#9ca3af",e.lineWidth=2,e.fillStyle="#1f1f23",e.beginPath(),e.roundRect(55,180,130,100,[0,0,10,10]),e.fill();const L=e.createLinearGradient(55,190,55,280);L.addColorStop(0,`rgba(${T}, ${S}, ${A}, 0.3)`),L.addColorStop(1,`rgba(${T}, ${S}, ${A}, 0.6)`),e.fillStyle=L,e.beginPath(),e.moveTo(55,190);const E=t.bathTemp>=80?1.5:.4,R=t.bathTemp>=80?.08:.03,Y=Date.now()*R;for(let w=55;w<=185;w+=5){const X=190+Math.sin(w*.15+Y)*E;e.lineTo(w,X)}e.lineTo(185,280),e.lineTo(55,280),e.closePath(),e.fill(),t.bathTemp>35&&(e.strokeStyle=`rgba(239, 68, 68, ${Math.min(.4,(t.bathTemp-35)/120)})`,e.lineWidth=1,t.heatWaves.forEach(w=>{e.beginPath(),e.moveTo(w.x,w.y),e.quadraticCurveTo(w.x+Math.sin(w.y*.06)*4,w.y-w.length/2,w.x,w.y-w.length),e.stroke()})),t.bathTemp<=8&&(e.fillStyle="rgba(255, 255, 255, 0.6)",e.strokeStyle="rgba(147, 197, 253, 0.8)",e.lineWidth=.8,t.iceCubes.forEach(w=>{e.save(),e.translate(w.x,w.y),e.rotate(w.angle),e.beginPath(),e.roundRect(-w.size/2,-w.size/2,w.size,w.size,2),e.fill(),e.stroke(),e.restore()})),t.bathTemp>=80&&(e.fillStyle="rgba(255, 255, 255, 0.5)",e.strokeStyle="rgba(255, 255, 255, 0.7)",e.lineWidth=.5,t.bubbles.forEach(w=>{e.beginPath(),e.arc(w.x,w.y,w.r,0,Math.PI*2),e.fill(),e.stroke()})),e.strokeStyle="#4b5563",e.lineWidth=3,e.beginPath(),e.moveTo(54,180),e.lineTo(54,272),e.arcTo(54,281,65,281,8),e.lineTo(175,281),e.arcTo(186,281,186,272,8),e.lineTo(186,180),e.stroke()}function V(e){const T=8+t.wallThickness*8,S=120-T/2,A=120+T/2,L=e.createLinearGradient(S,20,A,20);L.addColorStop(0,"rgba(209, 213, 219, 0.85)"),L.addColorStop(.2,"rgba(255, 255, 255, 0.95)"),L.addColorStop(.5,"rgba(243, 244, 246, 0.2)"),L.addColorStop(.8,"rgba(255, 255, 255, 0.95)"),L.addColorStop(1,"rgba(156, 163, 175, 0.9)"),e.fillStyle=L,e.strokeStyle="rgba(156, 163, 175, 0.8)",e.lineWidth=1,e.beginPath(),e.moveTo(S,230),e.lineTo(S,25),e.arcTo(S,20,S+5,20,5),e.arcTo(A,20,A,25,5),e.lineTo(A,230),e.closePath(),e.fill(),e.stroke();const E=e.createRadialGradient(120-11*.2,250-11*.2,11*.1,120,250,11);E.addColorStop(0,"rgba(255, 255, 255, 0.7)"),E.addColorStop(1,"rgba(156, 163, 175, 0.3)"),e.fillStyle=E,e.strokeStyle="#6b7280",e.lineWidth=.8+t.wallThickness*.6,e.beginPath(),e.arc(120,250,11+.5+t.wallThickness*.5,0,Math.PI*2),e.fill(),e.stroke();const R=Math.min(T*.72,.6+t.capillaryBore*4.5);e.fillStyle="#111827",e.fillRect(120-R/2,28,R,202);const Y=t.liquidType==="mercury",w=Y?"#9ca3af":"#ef4444",X=Y?"#f3f4f6":"#fecaca",D=210,k=220,te=(D-40)/k,ee=D-Math.min(k,t.thermometerTemp)*te;e.fillStyle=w,e.fillRect(120-R/2,ee,R,230-ee),e.fillStyle=X,e.fillRect(120-R/6,ee,R/3,230-ee);const re=11*.9,ae=e.createRadialGradient(120-re*.2,250-re*.2,re*.1,120,250,re);Y?(ae.addColorStop(0,"#f3f4f6"),ae.addColorStop(.3,"#d1d5db"),ae.addColorStop(.8,"#9ca3af"),ae.addColorStop(1,"#4b5563")):(ae.addColorStop(0,"#fee2e2"),ae.addColorStop(.2,"#fca5a5"),ae.addColorStop(.7,"#ef4444"),ae.addColorStop(1,"#991b1b")),e.fillStyle=ae,e.beginPath(),e.arc(120,250,re,0,Math.PI*2),e.fill(),e.strokeStyle="#4b5563",e.lineWidth=.5,e.font="6.5px Arial",e.fillStyle="#a1a1aa";for(let ce=0;ce<=200;ce+=50){const me=D-ce*te;e.beginPath(),e.moveTo(S,me),e.lineTo(S+4,me),e.stroke(),e.fillText(`${ce}°C`,S-22,me+2.5)}}function j(e){const T=e.createLinearGradient(116,20,124,20);T.addColorStop(0,"#9ca3af"),T.addColorStop(.3,"#f3f4f6"),T.addColorStop(.7,"#d1d5db"),T.addColorStop(1,"#6b7280"),e.fillStyle=T,e.strokeStyle="#4b5563",e.lineWidth=1,e.beginPath(),e.roundRect(116,20,8,225,[0,0,3,3]),e.fill(),e.stroke(),e.lineWidth=1.5,e.strokeStyle="#ef4444",e.beginPath(),e.moveTo(118,20),e.bezierCurveTo(110,5,200,30,245,60),e.stroke(),e.strokeStyle="#111827",e.beginPath(),e.moveTo(122,20),e.bezierCurveTo(130,8,210,45,245,75),e.stroke();const S=245,A=40,L=135,E=90;e.fillStyle="#1e1b4b",e.strokeStyle="#4f46e5",e.lineWidth=2.5,e.beginPath(),e.roundRect(S,A,L,E,10),e.fill(),e.stroke(),e.fillStyle="#022c22",e.beginPath(),e.roundRect(S+10,A+12,L-20,34,4),e.fill(),e.font='bold 14px "Courier New"',e.fillStyle="#34d399",e.textAlign="right",e.fillText(t.currentResistance.toFixed(2)+" Ω",S+L-16,A+34),e.font="6px Arial",e.fillStyle="#a7f3d0",e.textAlign="left",e.fillText("PLATINUM RTD METER",S+14,A+21)}function U(e){e.strokeStyle="#111827",e.lineWidth=1.2,e.beginPath(),e.moveTo(118,20),e.lineTo(118,245),e.stroke(),e.beginPath(),e.moveTo(122,20),e.lineTo(122,245),e.stroke();const h=20,g=225,T=10,S=120-T/2,A=e.createLinearGradient(S,h,S+T,h);A.addColorStop(0,"rgba(229, 231, 235, 0.4)"),A.addColorStop(.3,"rgba(255, 255, 255, 0.8)"),A.addColorStop(.7,"rgba(243, 244, 246, 0.5)"),A.addColorStop(1,"rgba(209, 213, 219, 0.6)"),e.fillStyle=A,e.strokeStyle="rgba(156, 163, 175, 0.6)",e.lineWidth=.8,e.beginPath(),e.roundRect(S,h,T,g-h,[0,0,2,2]),e.fill(),e.stroke();const L=6,E=e.createRadialGradient(120-L*.2,245-L*.2,L*.1,120,245,L);E.addColorStop(0,"#4b5563"),E.addColorStop(.7,"#1f2937"),E.addColorStop(1,"#111827"),e.fillStyle=E,e.strokeStyle="#030712",e.lineWidth=1,e.beginPath(),e.arc(120,245,L,0,Math.PI*2),e.fill(),e.stroke(),e.lineWidth=1.5,e.strokeStyle="#ef4444",e.beginPath(),e.moveTo(118,20),e.bezierCurveTo(110,5,200,30,245,60),e.stroke(),e.strokeStyle="#111827",e.beginPath(),e.moveTo(122,20),e.bezierCurveTo(130,8,210,45,245,75),e.stroke();const R=245,Y=40,w=135,X=90;e.fillStyle="#111827",e.strokeStyle="#10b981",e.lineWidth=2.5,e.beginPath(),e.roundRect(R,Y,w,X,10),e.fill(),e.stroke(),e.fillStyle="#022c22",e.beginPath(),e.roundRect(R+10,Y+12,w-20,34,4),e.fill(),e.font='bold 14px "Courier New"',e.fillStyle="#34d399",e.textAlign="right",e.fillText(t.currentThermistorR.toFixed(2)+" kΩ",R+w-16,Y+34),e.font="6px Arial",e.fillStyle="#a7f3d0",e.textAlign="left",e.fillText("NTC THERMISTOR METER",R+14,Y+21)}function Z(e){const{gx:i,gy:d,gw:u,gh:h,tickFont:g,axisFont:T,dotR:S,yLabelX:A,xLabelY:L}=O();e.strokeStyle="#4b5563",e.lineWidth=1.4,e.beginPath(),e.moveTo(i,d),e.lineTo(i,d+h),e.lineTo(i+u,d+h),e.stroke(),e.strokeStyle="#27272a",e.lineWidth=.6,e.fillStyle="#a1a1aa",e.font=g,e.textAlign="right",e.textBaseline="middle";const E=25,R=200;for(let k=0;k<=E;k+=5){const Q=d+h-k/E*h;e.beginPath(),e.moveTo(i,Q),e.lineTo(i+u,Q),e.stroke(),e.fillText(`${k.toFixed(1)}`,i-8,Q)}e.textAlign="center",e.textBaseline="top";for(let k=0;k<=R;k+=40){const Q=i+k/R*u;e.beginPath(),e.moveTo(Q,d),e.lineTo(Q,d+h),e.stroke(),e.fillText(`${k}`,Q,d+h+6)}e.save(),e.translate(A,d+h/2),e.rotate(-Math.PI/2),e.font=T,e.fillStyle="#e4e4e7",e.fillText("Length of liquid column / cm",0,0),e.restore(),e.font=T,e.fillText("temperature / °C",i+u/2,L);const Y=d+h-t.liquidL0/E*h,w=d+h-(t.liquidL0+(t.liquidL100-t.liquidL0)/100*R)/E*h;e.strokeStyle="#ef4444",e.lineWidth=2,e.beginPath(),e.moveTo(i,Y),e.lineTo(i+u,w),e.stroke();const X=t.thermometerTemp,D=t.currentLength;if(X>=0&&X<=R){const k=i+X/R*u,Q=d+h-D/E*h;e.fillStyle="#ef4444",e.beginPath(),e.arc(k,Q,S,0,Math.PI*2),e.fill()}}function B(e){const{gx:i,gy:d,gw:u,gh:h,tickFont:g,axisFont:T,dotR:S,yLabelX:A,xLabelY:L}=O();e.strokeStyle="#4b5563",e.lineWidth=1.4,e.beginPath(),e.moveTo(i,d),e.lineTo(i,d+h),e.lineTo(i+u,d+h),e.stroke(),e.strokeStyle="#27272a",e.lineWidth=.6,e.fillStyle="#a1a1aa",e.font=g,e.textAlign="right",e.textBaseline="middle";const E=10,R=200;for(let k=0;k<=E;k+=2){const Q=d+h-k/E*h;e.beginPath(),e.moveTo(i,Q),e.lineTo(i+u,Q),e.stroke(),e.fillText(`${k.toFixed(1)}`,i-8,Q)}e.textAlign="center",e.textBaseline="top";for(let k=0;k<=R;k+=40){const Q=i+k/R*u;e.beginPath(),e.moveTo(Q,d),e.lineTo(Q,d+h),e.stroke(),e.fillText(`${k}`,Q,d+h+6)}e.save(),e.translate(A,d+h/2),e.rotate(-Math.PI/2),e.font=T,e.fillStyle="#e4e4e7",e.fillText("Resistance of platinum / Ω",0,0),e.restore(),e.font=T,e.fillText("temperature / °C",i+u/2,L);const Y=d+h-t.resistanceR0/E*h,w=d+h-(t.resistanceR0+(t.resistanceR100-t.resistanceR0)/100*R)/E*h;e.strokeStyle="#4f46e5",e.lineWidth=2,e.beginPath(),e.moveTo(i,Y),e.lineTo(i+u,w),e.stroke();const X=t.thermometerTemp,D=t.currentResistance;if(X>=0&&X<=R){const k=i+X/R*u,Q=d+h-D/E*h;e.fillStyle="#4f46e5",e.beginPath(),e.arc(k,Q,S,0,Math.PI*2),e.fill()}}function K(e){const{gx:i,gy:d,gw:u,gh:h,tickFont:g,axisFont:T,dotR:S,yLabelX:A,xLabelY:L}=O();e.strokeStyle="#4b5563",e.lineWidth=1.4,e.beginPath(),e.moveTo(i,d),e.lineTo(i,d+h),e.lineTo(i+u,d+h),e.stroke(),e.strokeStyle="#27272a",e.lineWidth=.6,e.fillStyle="#a1a1aa",e.font=g,e.textAlign="right",e.textBaseline="middle";const E=20,R=200;for(let D=0;D<=E;D+=4){const k=d+h-D/E*h;e.beginPath(),e.moveTo(i,k),e.lineTo(i+u,k),e.stroke(),e.fillText(`${D.toFixed(1)}`,i-8,k)}e.textAlign="center",e.textBaseline="top";for(let D=0;D<=R;D+=40){const k=i+D/R*u;e.beginPath(),e.moveTo(k,d),e.lineTo(k,d+h),e.stroke(),e.fillText(`${D}`,k,d+h+6)}e.save(),e.translate(A,d+h/2),e.rotate(-Math.PI/2),e.font=T,e.fillStyle="#e4e4e7",e.fillText("Resistance of thermistor / kΩ",0,0),e.restore(),e.font=T,e.fillText("temperature / °C",i+u/2,L),e.strokeStyle="#10b981",e.lineWidth=2,e.beginPath();let Y=!1;for(let D=0;D<=R;D+=2){const k=D+273.15,Q=t.thermistorR25*Math.exp(t.thermistorBeta*(1/k-1/298.15)),te=i+D/R*u,ee=d+h-Math.min(E,Q)/E*h;ee>=d&&ee<=d+h&&(Y?e.lineTo(te,ee):(e.moveTo(te,ee),Y=!0))}e.stroke();const w=t.thermometerTemp,X=t.currentThermistorR;if(w>=0&&w<=R){const D=i+w/R*u,k=d+h-Math.min(E,X)/E*h;e.fillStyle="#10b981",e.beginPath(),e.arc(D,k,S,0,Math.PI*2),e.fill()}}function J(){$.clearRect(0,0,r,c),I.clearRect(0,0,b,v),$.save(),$.translate(o,0),P($),t.thermometerType==="liquid"?V($):t.thermometerType==="resistance"?j($):U($),$.restore(),t.thermometerType==="liquid"?Z(I):t.thermometerType==="resistance"?B(I):K(I)}function f(e){a.querySelector("#tl-bath-temp-display").innerHTML=t.bathTemp.toFixed(1)+"°C",a.querySelector("#tl-val-bath-temp").innerHTML=t.bathTemp.toFixed(1)+" °C";const i=a.querySelector("#tl-bath-state");t.bathTemp<=0?i.textContent="Melting Ice Bath":t.bathTemp>=100?i.textContent="Boiling Water/Steam":t.bathTemp===150?i.textContent="Hot Cooking Oil":i.textContent="Liquid Water",a.querySelector("#tl-val-response-time").textContent=e.toFixed(2)+" s";const d=a.querySelector("#tl-alcohol-boiling-warning");if(t.thermometerType==="liquid"&&t.liquidType==="alcohol"&&t.bathTemp>=78?d.style.display="block":d.style.display="none",t.thermometerType==="liquid")a.querySelector("#tl-live-liquid-lt").textContent=t.currentLength.toFixed(2)+" cm",a.querySelector("#tl-live-liquid-sub-num").textContent=`${t.currentLength.toFixed(2)} - ${t.liquidL0.toFixed(1)}`,a.querySelector("#tl-live-liquid-sub-den").textContent=`${t.liquidL100.toFixed(1)} - ${t.liquidL0.toFixed(1)}`,a.querySelector("#tl-live-liquid-ans").textContent=t.thermometerTemp.toFixed(1)+"°C";else if(t.thermometerType==="resistance")a.querySelector("#tl-live-resistance-rt").textContent=t.currentResistance.toFixed(2)+" Ω",a.querySelector("#tl-live-resistance-sub-num").textContent=`${t.currentResistance.toFixed(2)} - ${t.resistanceR0.toFixed(1)}`,a.querySelector("#tl-live-resistance-sub-den").textContent=`${t.resistanceR100.toFixed(1)} - ${t.resistanceR0.toFixed(1)}`,a.querySelector("#tl-live-resistance-ans").textContent=t.thermometerTemp.toFixed(1)+"°C";else{a.querySelector("#tl-live-thermistor-rt").textContent=t.currentThermistorR.toFixed(2)+" kΩ";const u=t.thermometerTemp+273.15;a.querySelector("#tl-live-thermistor-calc-tk").textContent=u.toFixed(2),a.querySelector("#tl-live-thermistor-ans").textContent=t.thermometerTemp.toFixed(1)+"°C"}}let y=null;function W(e){t.lastTimestamp||(t.lastTimestamp=e);const i=(e-t.lastTimestamp)/1e3;t.lastTimestamp=e;const d=Math.min(i,.1),u=F(),h=d/u*(t.bathTemp-t.thermometerTemp);t.thermometerTemp+=h,t.currentLength=t.liquidL0+(t.liquidL100-t.liquidL0)/100*t.thermometerTemp,t.currentResistance=t.resistanceR0+(t.resistanceR100-t.resistanceR0)/100*t.thermometerTemp;const g=t.thermometerTemp+273.15;t.currentThermistorR=t.thermistorR25*Math.exp(t.thermistorBeta*(1/g-1/298.15)),z(),J(),f(u),y=requestAnimationFrame(W)}function H(){const e=parseFloat(a.querySelector("#tl-input-faulty-cf").value),i=parseFloat(a.querySelector("#tl-input-faulty-cu").value),d=Number.isFinite(e)?e:-1.5,u=Number.isFinite(i)?i:105,h=u-d;return{cf:d,cu:u,interval:h}}function p(e){const i=Math.round(e*100)/100;return i>=0?i.toFixed(1):`(${i.toFixed(1)})`}function l(){const{cf:e,cu:i,interval:d}=H(),u=a.querySelector("#tl-val-faulty-interval"),h=a.querySelector("#tl-faulty-error-a"),g=a.querySelector("#tl-faulty-error-b"),T=a.querySelector("#tl-ans-q10a"),S=a.querySelector("#tl-ans-q10b"),A=Math.abs(d)<.01;if(u.textContent=d.toFixed(1)+" °C",A){h.hidden=!1,h.textContent="C_u must differ from C_f (interval cannot be zero).",g.hidden=!1,g.textContent=h.textContent,T.textContent="—",S.textContent="—";return}h.hidden=!0,g.hidden=!0;const L=parseFloat(a.querySelector("#tl-input-q10a-cm").value)||0,E=parseFloat(a.querySelector("#tl-input-q10b-t").value)||0,R=(L-e)/d*100,Y=E/100*d+e;a.querySelector("#tl-faulty-sub-num-a").textContent=`${L.toFixed(1)} − ${p(e)}`,a.querySelector("#tl-faulty-sub-den-a").textContent=`${i.toFixed(1)} − ${p(e)}`,a.querySelector("#tl-faulty-sub-num-b").textContent=`${E.toFixed(1)} × ${d.toFixed(1)}`,a.querySelector("#tl-faulty-sub-cf-b").textContent=`+ ${p(e)}`,T.textContent=R.toFixed(1)+" °C",S.textContent=Y.toFixed(2)+" °C"}function m(){const e=parseFloat(a.querySelector("#tl-input-q11-r").value)||0,i=(e-t.resistanceR0)/(t.resistanceR100-t.resistanceR0)*100;a.querySelector("#tl-formula-q11-rsub").textContent=e.toFixed(1),a.querySelector("#tl-ans-q11").textContent=i.toFixed(1)+" °C"}function x(){l(),m()}function q(e,i){a.querySelector("#"+e).addEventListener("click",()=>{t.bathTemp=i,a.querySelector("#tl-bath-temp-slider").value=i,x()})}function M(){a.querySelectorAll(".tl-tab-btn").forEach(i=>{i.addEventListener("click",()=>{a.querySelectorAll(".tl-tab-btn").forEach(u=>u.classList.remove("active")),a.querySelectorAll(".tl-tab-content").forEach(u=>u.classList.remove("active")),i.classList.add("active");const d=i.getAttribute("data-tl-tab");a.querySelector("#tl-tab-"+d).classList.add("active"),t.thermometerType=d,x()})}),a.querySelector("#tl-bath-temp-slider").addEventListener("input",i=>{t.bathTemp=parseFloat(i.target.value),x()}),q("tl-btn-preset-ice",0),q("tl-btn-preset-room",25),q("tl-btn-preset-steam",100),q("tl-btn-preset-oil",150),a.querySelector("#tl-card-mercury").addEventListener("click",()=>{t.liquidType="mercury",a.querySelector("#tl-card-mercury").className="tl-radio-card active-mercury",a.querySelector("#tl-card-alcohol").className="tl-radio-card",x()}),a.querySelector("#tl-card-alcohol").addEventListener("click",()=>{t.liquidType="alcohol",a.querySelector("#tl-card-mercury").className="tl-radio-card",a.querySelector("#tl-card-alcohol").className="tl-radio-card active-alcohol",x()}),a.querySelector("#tl-slider-bulb-vol").addEventListener("input",i=>{t.bulbVolume=parseFloat(i.target.value),a.querySelector("#tl-val-bulb-vol").textContent=t.bulbVolume+" mm³"}),a.querySelector("#tl-slider-wall-thick").addEventListener("input",i=>{t.wallThickness=parseFloat(i.target.value),a.querySelector("#tl-val-wall-thickness").textContent=t.wallThickness.toFixed(1)+" mm"}),a.querySelector("#tl-slider-capillary-bore").addEventListener("input",i=>{t.capillaryBore=parseFloat(i.target.value),a.querySelector("#tl-val-capillary-bore").textContent=t.capillaryBore.toFixed(2)+" mm"}),a.querySelector("#tl-slider-liquid-l0").addEventListener("input",i=>{t.liquidL0=parseFloat(i.target.value),a.querySelector("#tl-val-liquid-l0").textContent=t.liquidL0.toFixed(1)+" cm",x()}),a.querySelector("#tl-slider-liquid-l100").addEventListener("input",i=>{t.liquidL100=parseFloat(i.target.value),a.querySelector("#tl-val-liquid-l100").textContent=t.liquidL100.toFixed(1)+" cm",x()}),a.querySelector("#tl-slider-resistance-r0").addEventListener("input",i=>{t.resistanceR0=parseFloat(i.target.value),a.querySelector("#tl-val-resistance-r0").textContent=t.resistanceR0.toFixed(1)+" Ω",a.querySelector("#tl-spec-resistance-r0").textContent=t.resistanceR0.toFixed(1)+" Ω",x()}),a.querySelector("#tl-slider-resistance-r100").addEventListener("input",i=>{t.resistanceR100=parseFloat(i.target.value),a.querySelector("#tl-val-resistance-r100").textContent=t.resistanceR100.toFixed(1)+" Ω",a.querySelector("#tl-spec-resistance-r100").textContent=t.resistanceR100.toFixed(1)+" Ω",x()}),a.querySelector("#tl-slider-thermistor-r25").addEventListener("input",i=>{t.thermistorR25=parseFloat(i.target.value),a.querySelector("#tl-val-thermistor-r25").textContent=t.thermistorR25.toFixed(1)+" kΩ",a.querySelector("#tl-spec-thermistor-r25").textContent=t.thermistorR25.toFixed(1)+" kΩ",x()}),a.querySelector("#tl-slider-thermistor-beta").addEventListener("input",i=>{t.thermistorBeta=parseFloat(i.target.value),a.querySelector("#tl-val-thermistor-beta").textContent=t.thermistorBeta+" K",a.querySelector("#tl-spec-thermistor-beta").textContent=t.thermistorBeta+" K",x()}),a.querySelector("#tl-btn-solve-q10a").addEventListener("click",()=>{a.querySelector("#tl-btn-solve-q10a").classList.add("active"),a.querySelector("#tl-btn-solve-q10b").classList.remove("active"),a.querySelector("#tl-pane-q10a").classList.add("active"),a.querySelector("#tl-pane-q10b").classList.remove("active")}),a.querySelector("#tl-btn-solve-q10b").addEventListener("click",()=>{a.querySelector("#tl-btn-solve-q10a").classList.remove("active"),a.querySelector("#tl-btn-solve-q10b").classList.add("active"),a.querySelector("#tl-pane-q10a").classList.remove("active"),a.querySelector("#tl-pane-q10b").classList.add("active")}),a.querySelector("#tl-input-faulty-cf").addEventListener("input",l),a.querySelector("#tl-input-faulty-cu").addEventListener("input",l),a.querySelector("#tl-input-q10a-cm").addEventListener("input",l),a.querySelector("#tl-input-q10b-t").addEventListener("input",l),a.querySelector("#tl-input-q11-r").addEventListener("input",m)}return C(),M(),x(),y=requestAnimationFrame(W),a._thermometerLabCleanup=()=>{y&&cancelAnimationFrame(y)},a}const oe=[{id:"thermometer",titleKey:"topic.thermometer",fileEn:"thermometer-en.pdf",fileZh:"thermometer-zhHant.pdf",tool:"thermometerLab"},{id:"heatInternalEnergy",titleKey:"topic.heatInternalEnergy",fileEn:"heat-internal-energy-en.pdf",fileZh:"heat-internal-energy-zhHant.pdf"},{id:"changeOfState",titleKey:"topic.changeOfState",fileEn:"change-of-state-en.pdf",fileZh:"change-of-state-zhHant.pdf"},{id:"heatTransfer",titleKey:"topic.heatTransfer",fileEn:"heat-transfer-en.pdf",fileZh:"heat-transfer-zhHant.pdf"}],ge=["liquidThermometer","faultyThermometer","resistanceThermometer","thermistor"],st=[["thermometer","topic.thermometer"]];function ue(){return ie()==="zh-Hant"?"zhHant":"en"}async function lt(n,a){const t=`./${n}/${a}`;try{return(await fetch(t,{method:"HEAD"})).ok}catch{return!1}}async function we(n){return lt("notes",n)}function ct(n){let a="topics",t=0,r="all",c=!1,o=null,b={main:null},v=null;function O(){if(b.main){if(a==="topics"?b.main.innerHTML=$():a==="notes"?b.main.innerHTML=I():a==="tools"?b.main.innerHTML=C():a==="worksheets"?b.main.innerHTML=He(s,{topics:st,paperTitleKey:"worksheets.paperTitleHeat"}):a==="flashcards"?b.main.innerHTML=z():a==="summary"&&(b.main.innerHTML=Z()),a==="notes"&&N(),a==="tools"&&F(),a==="worksheets"){const f=_e.filter(y=>ge.includes(y.topic));Ae(n,f,s,ue,{topicFilter:(y,W)=>W.includes("thermometer")?ge.includes(y.topic):W.includes(y.topic)})}a==="flashcards"&&U(),a==="summary"&&B()}}function G(){o==null||o.destroy(),o=Re(n,{subtitleKey:"strand.heat.subtitle",activeSection:a,onSection:f=>{a=f,o.updateSection(a),O()},onLang:()=>G()}),b.main=o.main,o.updateSection(a),O()}function $(){return`
      <section class="panel panel--topic-hub">
        <h2>${s("topics.title")}</h2>
        <p class="lead">${s("topics.intro")}</p>
        <div class="grid cols-2 topic-hub-grid">
      ...
      </section>`}async function N(){const f=ue();for(const y of oe){const W=n.querySelector(`[data-note-card="${y.id}"]`);if(!W)continue;const H=W.querySelector("[data-note-body]"),p=f==="zhHant"?y.fileZh:y.fileEn,l=await we(p),m=`./notes/${p}`;l?H.innerHTML=`
      ...
      </section>`}async function B(){const f=ue();for(const y of oe){const W=n.querySelector(`[data-summary-card="${y.id}"]`);if(!W)continue;const H=W.querySelector("[data-summary-body]"),p=f==="zhHant"?y.fileZh:y.fileEn,l=await we(p),m=`./notes/${p}`;l?H.innerHTML=`
