...
`;function We(){if(document.getElementById(ue))return;const a=document.createElement("style");a.id=ue,a.textContent=Fe,document.head.appendChild(a)}function Pe(a){We();const n=document.createElement("div");n.className="rml",n.innerHTML=`
    <div class="rml-head">
      <h2 class="rml-title">${a("tools.rotatingMirror.title")}</h2>
      <div class="rml-sub">${a("tools.rotatingMirror.subtitle")}</div>
    </div>
    <div class="rml-dash">
      <div class="rml-viz">
        <canvas class="rml-canvas" width="700" height="420" aria-label="${a("tools.rotatingMirror.title")}"></canvas>
      </div>
      <div class="rml-controls">
        <div class="rml-toggles">
          <button type="button" class="rml-toggle rml-btn-green active" data-rml-toggle="orig-norm">
            <span class="rml-ind"></span> ${a("tools.rotatingMirror.toggle.origNormal")}
          </button>
          <button type="button" class="rml-toggle rml-btn-green active" data-rml-toggle="orig-ray">
            <span class="rml-ind"></span> ${a("tools.rotatingMirror.toggle.origRay")}
          </button>
          <button type="button" class="rml-toggle rml-btn-cyan active" data-rml-toggle="new-norm">
            <span class="rml-ind"></span> ${a("tools.rotatingMirror.toggle.newNormal")}
          </button>
          <button type="button" class="rml-toggle rml-btn-cyan active" data-rml-toggle="new-ray">
            <span class="rml-ind"></span> ${a("tools.rotatingMirror.toggle.newRay")}
          </button>
        </div>
        <div class="rml-cg">
          <div class="rml-lr">
            <span>${a("tools.rotatingMirror.incidentAngle")}</span>
        ...
        <div class="rml-cg">
          <div class="rml-lr">
            <span style="color:var(--rml-yellow)">${a("tools.rotatingMirror.mirrorRotation")}</span>
        ...
        <div class="rml-stats">
          <div class="rml-sr">
            <span class="rml-sl">${a("tools.rotatingMirror.statIncidence")}</span>
            <span class="rml-sv" style="color:var(--rml-yellow)" data-rml-val="i">50.0°</span>
          </div>
          <div class="rml-sr">
            <span class="rml-sl">${a("tools.rotatingMirror.statReflection")}</span>
            <span class="rml-sv" style="color:var(--rml-cyan)" data-rml-val="r">50.0°</span>
          </div>
        </div>
        <div class="rml-shift">
          <div>
            <div class="rml-sl" style="color:#ff8a80;margin-bottom:2px">${a("tools.rotatingMirror.rayShift")}</div>
            <div class="rml-shv" data-rml-val="shift">0.0°</div>
          </div>
          <div class="rml-logic">${a("tools.rotatingMirror.logicBadge")}</div>
        </div>
        <button type="button" class="rml-reset" data-rml-reset>${a("tools.rotatingMirror.reset")}</button>
      </div>
    </div>
  `;const t=n.querySelector(".rml-canvas"),i=t.getContext("2d"),r=n.querySelector("[data-rml-rot]"),f=n.querySelector("[data-rml-inc]"),q=n.querySelector('[data-rml-disp="rot"]'),T=n.querySelector('[data-rml-disp="inc"]'),E=n.querySelector('[data-rml-val="i"]'),$=n.querySelector('[data-rml-val="r"]'),I=n.querySelector('[data-rml-val="shift"]'),_=350,F=380,O=300,y="#22d3ee",L="bold 17px system-ui, sans-serif";let M=0,D=-140,Q=!0,W=!0,N=!0,B=!0;function P(R){return R*Math.PI/180}function j(R,c,l,m,e,o=2,d=!1){i.beginPath(),i.lineWidth=o,i.strokeStyle=e,i.setLineDash(d?[6,6]:[]);const u=R+Math.cos(P(l))*m,w=c+Math.sin(P(l))*m;return i.moveTo(R,c),i.lineTo(u,w),i.stroke(),i.setLineDash([]),{x:u,y:w}}function V(R,c,l,m,e,o=14){const d=l-R,u=m-c,w=Math.hypot(d,u);if(w<1e-6)return;const h=d/w,p=u/w,H=l-h*o,b=m-p*o,S=.45*o;i.beginPath(),i.fillStyle=e,i.moveTo(l,m),i.lineTo(H-p*S,b+h*S),i.lineTo(H+p*S,b-h*S),i.closePath(),i.fill()}function g(R,c,l,m,e,o=25){i.beginPath(),i.strokeStyle=m,i.lineWidth=2;const d=Math.min(R,c),u=Math.max(R,c);if(i.arc(_,F,l,P(d),P(u),!1),i.stroke(),e){const w=(d+u)/2,h=_+Math.cos(P(w))*(l+o),p=F+Math.sin(P(w))*(l+o);i.save(),i.font="bold 14px system-ui, sans-serif",i.textAlign="center",i.textBaseline="middle",i.fillStyle=m,i.fillText(e,h,p),i.restore()}}function k(R,c,l,m,e){i.clearRect(0,0,t.width,t.height),i.beginPath(),i.setLineDash([10,6]),i.strokeStyle="rgba(255, 204, 0, 0.45)",i.lineWidth=3,i.lineCap="round",i.moveTo(_-200,F),i.lineTo(_+200,F),i.stroke(),i.setLineDash([]),i.lineCap="butt";const o=_+Math.cos(P(R))*O,d=F+Math.sin(P(R))*O;i.beginPath(),i.lineWidth=3,i.strokeStyle="#fff",i.moveTo(o,d),i.lineTo(_,F),i.stroke();const u=(o+_)*.5,w=(d+F)*.5,h=Math.hypot(_-o,F-d)||1,p=(_-o)/h,H=(F-d)/h;if(V(u-p*28,w-H*28,u,w,"#fff",16),i.save(),i.font=L,i.textAlign="center",i.textBaseline="bottom",i.fillStyle="#fff",i.fillText(a("tools.rotatingMirror.canvas.incident"),o,d-12),i.restore(),Q){const b=j(_,F,-90,O-60,"#00e676",1,!0);i.save(),i.font="bold 14px system-ui, sans-serif",i.textAlign="left",i.textBaseline="bottom",i.fillStyle="#00e676",i.fillText(a("tools.rotatingMirror.canvas.n"),b.x+6,b.y-4),i.restore()}if(W){const b=j(_,F,m,O,"#00e676",2,!1);V(_,F,b.x,b.y,"#00e676",16),i.save(),i.font=L,i.textAlign="center",i.textBaseline="bottom",i.fillStyle="#00e676",i.fillText(a("tools.rotatingMirror.canvas.originalReflected"),b.x,b.y-14),i.restore()}i.save(),i.translate(_,F),i.rotate(P(M)),i.beginPath(),i.lineWidth=6,i.strokeStyle="#ffcc00",i.lineCap="round",i.moveTo(-200,0),i.lineTo(200,0),i.stroke(),i.beginPath(),i.lineWidth=1,i.strokeStyle="#666";for(let b=-190;b<200;b+=15)i.moveTo(b,0),i.lineTo(b-5,8);if(i.stroke(),i.restore(),N){const b=j(_,F,c,O-40,y,2,!0);i.save(),i.font="bold 14px system-ui, sans-serif",i.textAlign="left",i.textBaseline="bottom",i.fillStyle=y,i.fillText(a("tools.rotatingMirror.canvas.nPrime"),b.x+6,b.y-4),i.restore()}if(B){const b=j(_,F,l,O,y,4);V(_,F,b.x,b.y,y,16),Math.abs(M)>.5&&(i.save(),i.font=L,i.textAlign="center",i.textBaseline="bottom",i.fillStyle=y,i.fillText(a("tools.rotatingMirror.canvas.newReflected"),b.x,b.y-14),i.restore())}if(N&&g(R,c,100,"#ffcc00",`i=${e.toFixed(0)}°`,25),N&&B&&g(c,l,100,y,`r=${e.toFixed(0)}°`,25),Q&&N&&Math.abs(M)>.5&&g(-90,c,50,"#ffcc00",`θ=${Math.abs(M)}°`,20),W&&B&&Math.abs(M)>.5){const b=l-m;g(m,l,260,"#ff5252",`${a("tools.rotatingMirror.canvas.shift")}=${b.toFixed(0)}°`,40)}}function A(){M=parseFloat(r.value),D=parseFloat(f.value);const R=M>0?"+":"";q.textContent=`${R}${M}°`,T.textContent=`${D}°`;const c=-90,l=Math.abs(D-c),m=c+l,e=-90+M,o=Math.abs(D-e),d=e+o,u=d-m;E.textContent=`${o.toFixed(1)}°`,$.textContent=`${o.toFixed(1)}°`;const w=u>0?"+":"";I.textContent=`${w}${u.toFixed(1)}°`,k(D,e,d,m,o)}return n.querySelectorAll("[data-rml-toggle]").forEach(R=>{R.addEventListener("click",()=>{const c=R.getAttribute("data-rml-toggle");c==="orig-norm"&&(Q=!Q),c==="orig-ray"&&(W=!W),c==="new-norm"&&(N=!N),c==="new-ray"&&(B=!B),R.classList.toggle("active"),A()})}),n.querySelector("[data-rml-reset]").addEventListener("click",()=>{r.value="0",A()}),r.addEventListener("input",A),f.addEventListener("input",A),A(),n}function De(a){const n=document.createElement("div");n.className="tool-tir-escape";const t=document.createElement("iframe"),i="./",r=i.endsWith("/")?i:`${i}/`;return t.src=`${r}tir-escape/index.html?embed=1`,t.title=a("tools.refractionTir.title"),t.setAttribute("loading","lazy"),t.referrerPolicy="strict-origin-when-cross-origin",n.appendChild(t),n}function Qe(a,n={}){const t=n.defaultKind==="concave"?"concave":"convex",i="tools.lens.title",r=document.createElement("div");r.className="tool-lens-simulator";const f=document.createElement("iframe"),q="./",T=q.endsWith("/")?q:`${q}/`;return f.src=`${T}lens-simulator/index.html?embed=1&defaultLens=${encodeURIComponent(t)}`,f.title=a(i),f.setAttribute("loading","lazy"),f.referrerPolicy="strict-origin-when-cross-origin",r.appendChild(f),r}function je(a){const n=document.createElement("div");n.className="tool-em-spectrum";const t=document.createElement("iframe"),i="./",r=i.endsWith("/")?i:`${i}/`;function f(){return`${r}em-spectrum/index.html?lang=${encodeURIComponent(J())}`}t.src=f(),t.title=a("tools.em.title"),t.setAttribute("loading","lazy"),t.referrerPolicy="strict-origin-when-cross-origin";const q=()=>{var T;try{(T=t.contentWindow)==null||T.postMessage({type:"s3phy:lang",lang:J()},"*")}catch{t.src=f()}};return window.addEventListener("s3phy:lang",q),n._emLabCleanup=()=>window.removeEventListener("s3phy:lang",q),n.appendChild(t),n}function Oe(a){const n=document.createElement("div");n.className="rgb-mixer",n.innerHTML=`
  ...
  `;const t=n.querySelector("[data-r]"),i=n.querySelector("[data-g]"),r=n.querySelector("[data-b]"),f=n.querySelector(".rgb-mixer-canvas"),q=n.querySelector(".rgb-mixer-stage"),T=f.getContext("2d");function E(y){return y.toString(16).padStart(2,"0")}function $(y,L,M){const D=f.width/(window.devicePixelRatio||1),Q=f.height/(window.devicePixelRatio||1),W=D/2,N=Q/2,B=Math.min(D,Q)*.34,P=B*.5,j=Math.sqrt(3)/2;T.clearRect(0,0,D,Q),T.fillStyle="#000",T.fillRect(0,0,D,Q);const V=[{x:W,y:N-P,rgb:[255,0,0],val:y},{x:W-P*j,y:N+P*.5,rgb:[0,255,0],val:L},{x:W+P*j,y:N+P*.5,rgb:[0,0,255],val:M}];T.globalCompositeOperation="lighter",V.forEach(g=>{if(g.val<=0)return;const k=g.val/255,[A,R,c]=g.rgb,l=T.createRadialGradient(g.x,g.y,0,g.x,g.y,B);l.addColorStop(0,`rgba(${A},${R},${c},${k})`),l.addColorStop(.65,`rgba(${A},${R},${c},${k})`),l.addColorStop(.85,`rgba(${A},${R},${c},${k*.5})`),l.addColorStop(1,`rgba(${A},${R},${c},0)`),T.fillStyle=l,T.beginPath(),T.arc(g.x,g.y,B,0,Math.PI*2),T.fill()}),T.globalCompositeOperation="source-over"}function I(){const y=q.getBoundingClientRect(),L=Math.max(200,Math.min(y.width-8,y.height-8,720)),M=window.devicePixelRatio||1;f.width=L*M,f.height=L*M,f.style.width=`${L}px`,f.style.height=`${L}px`,T.setTransform(M,0,0,M,0,0),$(+t.value,+i.value,+r.value)}function _(){const y=+t.value,L=+i.value,M=+r.value,D=`rgb(${y}, ${L}, ${M})`,Q=`#${E(y)}${E(L)}${E(M)}`;n.querySelector("[data-r-val]").textContent=y,n.querySelector("[data-g-val]").textContent=L,n.querySelector("[data-b-val]").textContent=M,n.querySelector("[data-r-bar]").style.width=`${y/255*100}%`,n.querySelector("[data-g-bar]").style.width=`${L/255*100}%`,n.querySelector("[data-b-bar]").style.width=`${M/255*100}%`,t.style.setProperty("--fill",`${y/255*100}%`),i.style.setProperty("--fill",`${L/255*100}%`),r.style.setProperty("--fill",`${M/255*100}%`);const W=n.querySelector("[data-swatch]");W.style.backgroundColor=D,W.setAttribute("aria-label",`${a("tools.rgbMixer.preview")}: ${D}`),n.querySelector("[data-rgb-text]").textContent=D,n.querySelector("[data-hex-text]").textContent=Q,$(y,L,M)}function F(y,L,M){t.value=y,i.value=L,r.value=M,_()}return[t,i,r].forEach(y=>y.addEventListener("input",_)),n.querySelectorAll(".rgb-presets button").forEach(y=>{y.addEventListener("click",()=>{F(+y.dataset.r,+y.dataset.g,+y.dataset.b)})}),new ResizeObserver(()=>I()).observe(q),requestAnimationFrame(()=>{I(),_()}),n}const Ne=["topics","notes","tools","worksheets","flashcards","summary"],Be={topics:"nav.topics",notes:"nav.notes",tools:"nav.tools",worksheets:"nav.worksheets",flashcards:"nav.flashcards",summary:"nav.summary"};function qe(a,{subtitleKey:n,activeSection:t,onSection:i,onLang:r}){a.innerHTML=`
  ...
  `;const f=a.querySelector("[data-main]"),q=a.querySelector("[data-nav]"),T=a.querySelector("[data-lang]"),E=a.querySelector("[data-strand-back]");let $=t;function I(y){$=y,q.innerHTML=Ne.map(L=>`<button type="button" class="${y===L?"active":""}" data-sec="${L}">${s(Be[L])}</button>`).join(""),q.querySelectorAll("button").forEach(L=>{L.addEventListener("click",()=>i(L.getAttribute("data-sec")))})}function _(){T.innerHTML=`
      <button type="button" data-set-lang="en" class="${J()==="en"?"active":""}">${s("lang.en")}</button>
      <button type="button" data-set-lang="zh-Hant" class="${J()==="zh-Hant"?"active":""}">${s("lang.zhHant")}</button>
    `,T.querySelectorAll("button").forEach(y=>{y.addEventListener("click",()=>{ye(y.getAttribute("data-set-lang")),r()})})}function F(){a.querySelector("[data-hub-subtitle]").textContent=s(n),E.textContent=s("strand.back"),a.querySelector("[data-hub-footer]").textContent=s("footer.conventions"),a.querySelector(".site-title").textContent=s("app.title")}const O=()=>{window.dispatchEvent(new CustomEvent("s3phy:strand",{detail:null}))};return E.addEventListener("click",O),I(t),_(),{main:f,updateSection(y){I(y)},refreshLabels(){F(),I($),_()},destroy(){E.removeEventListener("click",O)}}}const de=["A","B","C","D"];function he(a){const n=a.slice();for(let t=n.length-1;t>0;t-=1){const i=Math.floor(Math.random()*(t+1));[n[t],n[i]]=[n[i],n[t]]}return n}function Ve(a,n){return a.filter(t=>{const i=t.topic;return!!(n.includes(i)||n.includes("rotatingMirror")&&i==="reflection"||n.includes("refraction")&&i==="refraction"||n.includes("tir")&&i==="tir"||i==="convex"&&n.includes("convex")||i==="concave"&&n.includes("concave"))})}function Ge(){return new Date().toLocaleDateString(void 0,{year:"numeric",month:"long",day:"numeric"})}function Z(a){return String(a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Se(a){const n=[["rotatingMirror","topic.rotatingMirror"],["refraction","topic.refractionSnell"],["tir","topic.tir"],["convex","topic.convex"],["concave","topic.concave"],["em","topic.em"]];return`
    <section class="panel panel--worksheets">
      <h2>${a("worksheets.title")}</h2>
      <p class="lead">${a("worksheets.intro")}</p>
      <div class="ws-controls no-print">
        <div class="controls">
          <div class="control">
            <label>${a("worksheets.count")}</label>
            <select data-ws-count>
              <option>5</option><option>10</option><option>15</option>
            </select>
          </div>
        </div>
        <p class="lead" style="margin-top:10px">${a("worksheets.topics")}</p>
        <div class="grid cols-2" data-ws-topics>
          ${n.map(([t,i])=>`<label class="card ws-topic-card">
            <input type="checkbox" data-ws-topic="${t}" checked />
            <span>${a(i)}</span>
          </label>`).join("")}
        </div>
        <p class="ws-action-row">
          <button class="btn primary" type="button" data-ws-gen>${a("worksheets.generate")}</button>
        </p>
      </div>
      <div class="ws-preview-area" data-ws-preview-area hidden>
        <div class="preview-tabs no-print" data-ws-tabs>
          <button type="button" class="preview-tab active" data-ws-tab="practice">${a("worksheets.tabPractice")}</button>
          <button type="button" class="preview-tab" data-ws-tab="answer">${a("worksheets.tabAnswer")}</button>
        </div>
        <div class="worksheet-paper practice-mode" data-ws-paper>
          <div class="worksheet-header">
            <div class="header-top">
              <h3>${a("worksheets.paperTitle")}</h3>
              <div class="ws-date-row no-print" data-ws-date-row>
                <span data-ws-date-label></span>
                <button type="button" class="btn btn-sm" data-ws-date-today>${a("worksheets.today")}</button>
              </div>
            </div>
        ...
        <div class="practice-actions no-print" data-ws-practice-actions>
          <div class="score-display" data-ws-score hidden>
            <span class="score-label">${a("worksheets.score")}</span>
        ...
        <div class="export-buttons no-print" data-ws-export>
          <button type="button" class="export-btn" data-ws-word-p>${a("worksheets.exportWord")}</button>
          <button type="button" class="export-btn" data-ws-word-a>${a("worksheets.exportWordAnswers")}</button>
          <button type="button" class="export-btn" data-ws-print-p>${a("worksheets.printPractice")}</button>
          <button type="button" class="export-btn" data-ws-print-a>${a("worksheets.printAnswers")}</button>
        </div>
      </div>
      <div class="worksheet-output" data-ws-out><p class="lead">${a("worksheets.empty")}</p></div>
    </section>`}function Te(a,n,t,i){var l,m,e,o;const r={items:[],tab:"practice",dateStr:"",userAnswers:{},wrongAttempts:{},resolved:{}},f=a.querySelector("[data-ws-gen]"),q=a.querySelector("[data-ws-preview-area]"),T=a.querySelector("[data-ws-out]"),E=a.querySelector("[data-ws-body]"),$=a.querySelector("[data-ws-paper]"),I=a.querySelector("[data-ws-tabs]"),_=a.querySelector("[data-ws-practice-actions]"),F=a.querySelector("[data-ws-score]"),O=a.querySelector("[data-ws-score-val]"),y=a.querySelector("[data-ws-summary]"),L=a.querySelector("[data-ws-date-row]"),M=a.querySelector("[data-ws-date-print]"),D=a.querySelector("[data-ws-date-today]"),Q=a.querySelector("[data-ws-date-label]");if(!f||!E)return;function W(){return i()}function N(){r.userAnswers={},r.wrongAttempts={},r.resolved={},F.hidden=!0,y.hidden=!0}function B(){return r.items.length>0&&r.items.every((d,u)=>r.resolved[u])}function P(){let d=0;r.items.forEach((w,h)=>{if(!r.resolved[h])return;const p=w[W()]||w.en;r.userAnswers[h]===p.a&&(d+=1)}),Object.keys(r.resolved).length>0&&(O.textContent=`${d}/${r.items.length}`,F.hidden=!1)}function j(d){r.tab=d,I==null||I.querySelectorAll("[data-ws-tab]").forEach(u=>{u.classList.toggle("active",u.getAttribute("data-ws-tab")===d)}),_&&(_.hidden=d!=="practice"),L&&(L.hidden=d!=="practice"),M&&(M.hidden=!r.dateStr,M.textContent=r.dateStr?`${t("worksheets.date")}: ${r.dateStr}`:""),$==null||$.classList.toggle("practice-mode",d==="practice"),g()}function V(d,u,w){const h=d[W()]||d.en,p=de,H=u+1,b=d.section?`<span class="q-section">${Z(d.section)}</span>`:"";if(w==="answer"){const te=h.choices.map((Y,K)=>`<li class="${K===h.a?"choice-correct":""}"><strong>${p[K]}.</strong> ${Z(Y)}</li>`).join("");return`<div class="question-row question-row--answer">
        <h4>Q${H} ${b}</h4>
        <p>${Z(h.q)}</p>
        <ol class="choice-list">${te}</ol>
        <p class="q-exp"><strong>${t("worksheets.answer")}:</strong> ${p[h.a]}</p>
        <p class="q-exp"><em>${Z(h.exp)}</em></p>
      </div>`}const S=r.userAnswers[u],C=r.wrongAttempts[u]||0,v=r.resolved[u],z=v&&S===h.a,x=v&&S!==h.a;let G="";C===1&&!v?G=`<p class="q-hint"><strong>${t("worksheets.hint")}:</strong> ${Z(h.hint||h.exp)}</p>`:x&&(G=`<p class="q-exp"><strong>${t("worksheets.answer")}:</strong> ${p[h.a]}</p>
        <p class="q-exp q-summary"><em>${Z(h.exp)}</em></p>`);const X=z?"correct":x?"incorrect":"",ee=h.choices.map((te,Y)=>`<li>
          <label class="choice-radio">
            <input type="radio" name="ws-q-${u}" value="${Y}" data-ws-q="${u}" data-ws-choice="${Y}"${S===Y?" checked":""}${v?" disabled":""} />
            <span class="choice-circle choice-circle--print"></span>
            <strong>${p[Y]}.</strong> ${Z(te)}
          </label>
        </li>`).join("");return`<div class="question-row ${X}" data-ws-row="${u}">
      <div class="q-header"><h4>Q${H} ${b}</h4>${z?'<span class="result-icon">✓</span>':x?'<span class="result-icon">✗</span>':""}</div>
      <p>${Z(h.q)}</p>
      <ol class="choice-list choice-list--radio">${ee}</ol>
      ${G}
    </div>`}function g(){r.items.length&&(E.innerHTML=r.items.map((d,u)=>V(d,u,r.tab)).join(""),k())}function k(){r.tab==="practice"&&E.querySelectorAll("[data-ws-choice]").forEach(d=>{d.addEventListener("change",()=>{const u=Number(d.getAttribute("data-ws-q"));if(r.resolved[u])return;const w=Number(d.getAttribute("data-ws-choice")),h=r.items[u][W()]||r.items[u].en;r.userAnswers[u]=w,w===h.a?r.resolved[u]=!0:(r.wrongAttempts[u]=(r.wrongAttempts[u]||0)+1,r.wrongAttempts[u]>=2&&(r.resolved[u]=!0)),P(),B()&&A(),g()})})}function A(){const d=r.items.length;let u=0,w=0;const h={};r.items.forEach((S,C)=>{const v=S[W()]||S.en;if(r.userAnswers[C]===v.a)u+=1,(r.wrongAttempts[C]||0)===0&&(w+=1);else if(r.resolved[C]){const x=S.section||S.topic;h[x]=(h[x]||0)+1}});const p=d?Math.round(u/d*100):0;let H=t("worksheets.revisionFair");p>=90?H=t("worksheets.revisionExcellent"):p>=70?H=t("worksheets.revisionGood"):p<50&&(H=t("worksheets.revisionLow"));const b=Object.entries(h).sort((S,C)=>C[1]-S[1]).map(([S,C])=>`<li>${Z(S)} (${C})</li>`).join("");y.innerHTML=`
      <h4>${t("worksheets.sessionSummary")}</h4>
      <p>${t("worksheets.score")}: <strong>${u}/${d}</strong> (${p}%)</p>
      <p>${t("worksheets.firstTry")}: <strong>${w}/${d}</strong></p>
      <p>${t("worksheets.revisionSuggestions")}: ${H}</p>
      ${b?`<p>${t("worksheets.weakSections")}:</p><ul>${b}</ul>`:""}`,y.hidden=!1}function R(d){const u=W(),w=r.items.map((h,p)=>{const H=h[u]||h.en,b=H.choices.map((C,v)=>`${de[v]}. ${C}`).join("<br/>"),S=d?`<p><b>${t("worksheets.answer")}:</b> ${de[H.a]}<br/><i>${H.exp}</i></p>`:"";return`<div style="page-break-inside:avoid;margin-bottom:16px"><b>Q${p+1}.</b> ${H.q}<br/>${b}${S}</div>`}).join("");return`<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word">
<head><meta charset="utf-8"><title>${t("worksheets.paperTitle")}</title></head>
<body><h2>${t("worksheets.paperTitle")}</h2>
${r.dateStr&&!d?`<p>${t("worksheets.date")}: ${r.dateStr}</p>`:""}
${w}</body></html>`}function c(d){const u=R(d),w=new Blob(["\uFEFF",u],{type:"application/msword"}),h=document.createElement("a");h.href=URL.createObjectURL(w),h.download=d?"worksheet-answers.doc":"worksheet.doc",h.click(),URL.revokeObjectURL(h.href)}f.addEventListener("click",()=>{const d=Number(a.querySelector("[data-ws-count]").value),u=[...a.querySelectorAll("[data-ws-topic]")].filter(p=>p.checked).map(p=>p.getAttribute("data-ws-topic")),w=Ve(n,u),h=he(w).slice(0,Math.min(d,w.length||1));r.items=h.length?h:he(n).slice(0,Math.min(d,n.length)),N(),r.tab="practice",r.dateStr="",q.hidden=!1,T.hidden=!0,I==null||I.querySelectorAll("[data-ws-tab]").forEach(p=>{p.classList.toggle("active",p.getAttribute("data-ws-tab")==="practice")}),_&&(_.hidden=!1),L&&(L.hidden=!1),M&&(M.hidden=!0),Q.textContent="",$==null||$.classList.add("practice-mode"),g()}),I==null||I.addEventListener("click",d=>{const u=d.target.closest("[data-ws-tab]");!u||!r.items.length||j(u.getAttribute("data-ws-tab"))}),D==null||D.addEventListener("click",()=>{r.dateStr?(r.dateStr="",Q.textContent=""):(r.dateStr=Ge(),Q.textContent=`${t("worksheets.date")}: ${r.dateStr}`),M&&(M.hidden=!r.dateStr,M.textContent=r.dateStr?`${t("worksheets.date")}: ${r.dateStr}`:"")}),(l=a.querySelector("[data-ws-word-p]"))==null||l.addEventListener("click",()=>c(!1)),(m=a.querySelector("[data-ws-word-a]"))==null||m.addEventListener("click",()=>c(!0)),(e=a.querySelector("[data-ws-print-p]"))==null||e.addEventListener("click",()=>{j("practice"),$==null||$.classList.add("ws-print-blank");const d=()=>$==null?void 0:$.classList.remove("ws-print-blank");window.addEventListener("afterprint",d,{once:!0}),window.print()}),(o=a.querySelector("[data-ws-print-a]"))==null||o.addEventListener("click",()=>{j("answer"),window.print()})}const Ye=["rotatingMirror","refractionTir","lens","rgbMixer","em"],Xe={rotatingMirror:a=>Pe(a),refractionTir:a=>De(a),lens:(a,n)=>Qe(a,{defaultKind:n}),rgbMixer:a=>Oe(a),em:a=>je(a)};function Ue(a){return s({rotatingMirror:"tools.rotatingMirror.title",refractionTir:"tools.refractionTir.title",lens:"tools.lens.title",rgbMixer:"tools.rgbMixer.title",em:"tools.em.title"}[a]||a)}function re(){return J()==="zh-Hant"?"zhHant":"en"}async function ke(a,n){const t=`./${a}/${n}`;try{return(await fetch(t,{method:"HEAD"})).ok}catch{return!1}}async function Ke(a){return ke("notes",a)}function Ze(a){let n="topics",t="rotatingMirror",i="convex",r=0,f="all",q=!1,T=null,E={main:null};function $(){E.main&&(n==="topics"?E.main.innerHTML=_():n==="notes"?E.main.innerHTML=O():n==="tools"?E.main.innerHTML=L():n==="worksheets"?E.main.innerHTML=Se(s):n==="flashcards"?E.main.innerHTML=Q():n==="summary"&&(E.main.innerHTML=g()),n==="notes"&&y(),n==="tools"&&M(),n==="worksheets"&&Te(a,we,s,re),n==="flashcards"&&V(),n==="summary"&&k())}function I(){T==null||T.destroy(),T=qe(a,{subtitleKey:"strand.optics.subtitle",activeSection:n,onSection:c=>{n=c,T.updateSection(n),$()},onLang:()=>I()}),E.main=T.main,T.updateSection(n),$()}function _(){const c=[["rotatingMirror","topic.reflection"],["refraction","topic.refractionSnell"],["tir","topic.tir"],["convex","topic.convex"],["concave","topic.concave"],["em","topic.em"]];return`
      <section class="panel panel--topic-hub">
        <h2>${s("topics.title")}</h2>
        <p class="lead">${s("topics.intro")}</p>
        <div class="grid cols-2 topic-hub-grid">
          ${c.map(([l,m])=>{const e=l==="convex"||l==="concave"?"lens":l==="refraction"||l==="tir"?"refractionTir":l==="em"?"em":l,o=l==="convex"?"convex":l==="concave"?"concave":"";return`
            <div class="card">
              <h3>${s(m)}</h3>
              <button class="btn primary" type="button" data-go-tool="${e}"${o?` data-lens-kind="${o}"`:""}>${s("topic.openTool")}</button>
            </div>`}).join("")}
        </div>
      </section>`}function F(c){const l=c.target.closest("[data-go-tool]");if(!l)return;n="tools",t=l.getAttribute("data-go-tool");const m=l.getAttribute("data-lens-kind");(m==="convex"||m==="concave")&&(i=m),T.updateSection(n),$()}function O(){const c=[{key:"reflection",fileEn:"reflection-en.pdf",fileZh:"reflection-zhHant.pdf"},{key:"refraction",fileEn:"refraction-en.pdf",fileZh:"refraction-zhHant.pdf"},{key:"tir",fileEn:"tir-en.pdf",fileZh:"tir-zhHant.pdf"},{key:"convexLens",fileEn:"convex-lens-en.pdf",fileZh:"convex-lens-zhHant.pdf"},{key:"concaveLens",fileEn:"concave-lens-en.pdf",fileZh:"concave-lens-zhHant.pdf"},{key:"em",fileEn:"emwaves-en.pdf",fileZh:"emwaves-zhHant.pdf"}];return`
      <section class="panel">
        <h2>${s("notes.title")}</h2>
        <p class="lead">${s("notes.intro")}</p>
        <p class="lead">${s("notes.embedHint")}</p>
        <div class="grid cols-2" data-notes-grid>
          ${c.map(l=>{const m=s(`notes.card.${l.key}`);return`
            <div class="card" data-note-card="${l.key}">
              <h3>${m}</h3>
              <div data-note-body></div>
            </div>`}).join("")}
        </div>
      </section>`}async function y(){const c=[{key:"reflection",fileEn:"reflection-en.pdf",fileZh:"reflection-zhHant.pdf"},{key:"refraction",fileEn:"refraction-en.pdf",fileZh:"refraction-zhHant.pdf"},{key:"tir",fileEn:"tir-en.pdf",fileZh:"tir-zhHant.pdf"},{key:"convexLens",fileEn:"convex-lens-en.pdf",fileZh:"convex-lens-zhHant.pdf"},{key:"concaveLens",fileEn:"concave-lens-en.pdf",fileZh:"concave-lens-zhHant.pdf"},{key:"em",fileEn:"emwaves-en.pdf",fileZh:"emwaves-zhHant.pdf"}],l=re();for(const m of c){const e=a.querySelector(`[data-note-card="${m.key}"]`);if(!e)continue;const o=e.querySelector("[data-note-body]"),d=l==="zhHant"?m.fileZh:m.fileEn,u=await Ke(d),w=`./notes/${d}`;u?o.innerHTML=`
          <iframe class="notes-grid" title="${s(`notes.card.${m.key}`)}" src="${w}"></iframe>
          <p style="margin-top:8px"><a href="${w}" target="_blank" rel="noopener">${s("notes.openPdf")}</a></p>`:o.innerHTML=`<p class="lead">${s("notes.missing")}</p>
      ...
      <section class="panel panel--tools">
        ...
        <div class="tools-layout">
          <div class="tool-list" data-tool-list>
            ${Ye.map(c=>`<button type="button" data-tool="${c}" class="${t===c?"active":""}">${Ue(c)}</button>`).join("")}
          </div>
          <div class="tool-stage" data-tool-stage></div>
        </div>
      </section>`}function M(){const c=a.querySelector("[data-tool-list]"),l=a.querySelector("[data-tool-stage]");!c||!l||(c.querySelectorAll("button").forEach(m=>{m.addEventListener("click",()=>{t=m.getAttribute("data-tool"),c.querySelectorAll("button").forEach(e=>e.classList.toggle("active",e.getAttribute("data-tool")===t)),D(l)})}),D(l))}function D(c){c.innerHTML="";const l=Xe[t];if(!l)return;const m=t==="lens"?l(s,i):l(s);c.appendChild(m)}function Q(){return`
      <section class="panel">
      ...
      </section>`}function W(c){return!!(c!=null&&c.front)}function N(c){return`./${String(c).replace(/^\.\//,"")}`}function B(){return xe.slice()}function P(){const c=Ie.slice(),l=B(),m=f==="rotatingMirror"?"reflection":f;if(m==="all")return[...c,...l];if(m==="reflection")return c;if(m==="convex"||m==="concave"){const o=l.filter(d=>d.topic==="convex"||d.topic==="concave");return o.length?o:l}if(m==="refractionTir"){const o=l.filter(d=>d.topic==="refraction"||d.topic==="tir");return o.length?o:l}const e=l.filter(o=>o.topic===m);return e.length?e:l}function j(){const c=P(),l=a.querySelector("[data-flash-progress]");if(!c.length){r=0;const h=a.querySelector("[data-flash-front]"),p=a.querySelector("[data-flip-card]");h&&(h.textContent=""),p&&p.classList.remove("flashcard-surface--image"),l&&(l.hidden=!0,l.textContent="");return}r=Math.max(0,Math.min(r,c.length-1));const m=c[r],e=a.querySelector("[data-flash-front]"),o=a.querySelector("[data-flip-card]"),d=a.querySelector("[data-flash-label]");if(!e||!o)return;if(l&&(l.hidden=!1,l.textContent=s("flashcards.progress").replace("{current}",String(r+1)).replace("{total}",String(c.length))),W(m)){const h=m.back&&m.back!==m.front;o.classList.add("flashcard-surface--image"),d&&(h?(d.hidden=!1,d.textContent=s(q?"flashcards.answer":"flashcards.question")):d.hidden=!0);const p=q&&m.back?m.back:m.front,H=m.alt||m.title||"";e.innerHTML=`<img src="${N(p)}" alt="${H.replace(/"/g,"&quot;")}" loading="lazy" />`;return}o.classList.remove("flashcard-surface--image");const u=re(),w=m[u]||m.en;d&&(d.hidden=!1,d.textContent=s(q?"flashcards.answer":"flashcards.question")),e.textContent=q?w.a:w.q}function V(){f==="rotatingMirror"&&(f="reflection");const c=a.querySelector("[data-flash-deck]");c.value=f,c.addEventListener("change",()=>{f=c.value,r=0,q=!1,j()}),a.querySelector("[data-flip-card]").addEventListener("click",()=>{q=!q,j()}),a.querySelector("[data-flash-flip]").addEventListener("click",l=>{l.stopPropagation(),q=!q,j()}),a.querySelector("[data-flash-prev]").addEventListener("click",()=>{const l=P();r=(r-1+l.length)%l.length,q=!1,j()}),a.querySelector("[data-flash-next]").addEventListener("click",()=>{const l=P();r=(r+1)%l.length,q=!1,j()}),a.querySelector("[data-flash-shuf]").addEventListener("click",()=>{const l=P();r=Math.floor(Math.random()*l.length),q=!1,j()}),j()}function g(){const c=[{key:"reflection",fileEn:"reflection-en.pdf",fileZh:"reflection-zhHant.pdf"},{key:"refraction",fileEn:"refraction-en.pdf",fileZh:"refraction-zhHant.pdf"},{key:"tir",fileEn:"tir-en.pdf",fileZh:"tir-zhHant.pdf"},{key:"convex",fileEn:"convex-en.pdf",fileZh:"convex-zhHant.pdf"},{key:"concave",fileEn:"concave-en.pdf",fileZh:"concave-zhHant.pdf"},{key:"em",fileEn:"em-en.pdf",fileZh:"em-zhHant.pdf"}];return`
      <section class="panel">
        <h2>${s("summary.title")}</h2>
        <p class="lead">${s("summary.intro")}</p>
        <p class="lead">${s("notes.embedHint")}</p>
        <div class="grid cols-2" data-summary-grid>
          ${c.map(l=>{const m=s(`summary.item.${l.key}`);return`
            <div class="card" data-summary-card="${l.key}">
              <h3>${m}</h3>
              <div data-summary-body></div>
            </div>`}).join("")}
        </div>
      </section>`}async function k(){const c=[{key:"reflection",fileEn:"reflection-en.pdf",fileZh:"reflection-zhHant.pdf"},{key:"refraction",fileEn:"refraction-en.pdf",fileZh:"refraction-zhHant.pdf"},{key:"tir",fileEn:"tir-en.pdf",fileZh:"tir-zhHant.pdf"},{key:"convex",fileEn:"convex-en.pdf",fileZh:"convex-zhHant.pdf"},{key:"concave",fileEn:"concave-en.pdf",fileZh:"concave-zhHant.pdf"},{key:"em",fileEn:"em-en.pdf",fileZh:"em-zhHant.pdf"}],l=re();for(const m of c){const e=a.querySelector(`[data-summary-card="${m.key}"]`);if(!e)continue;const o=e.querySelector("[data-summary-body]"),d=l==="zhHant"?m.fileZh:m.fileEn,u=await ke("summary-pdfs",d),w=`./summary-pdfs/${d}`;u?o.innerHTML=`
          <iframe class="notes-grid" title="${s(`summary.item.${m.key}`)}" src="${w}"></iframe>
          <p style="margin-top:8px"><a href="${w}" target="_blank" rel="noopener">${s("summary.download")}</a></p>`:o.innerHTML=`<p class="lead">${s("summary.missing")}</p>`}}const A=()=>I(),R=c=>F(c);return window.addEventListener("s3phy:lang",A),a.addEventListener("click",R),I(),()=>{window.removeEventListener("s3phy:lang",A),a.removeEventListener("click",R),T==null||T.destroy()}}const me="s3phy-thermometer-lab-css",Je=`
.tl-wrap {
  --tl-bg: #0d0d10;
  --tl-panel: #18181b;
  --tl-border: #27272a;
...
`;function et(){if(document.getElementById(me))return;const a=document.createElement("style");a.id=me,a.textContent=Je,document.head.appendChild(a)}function tt(a){et();const n=document.createElement("div");n.className="tl-wrap",n.innerHTML=`
    <div class="tl-head">
      <h2 class="tl-title">${a("tools.thermometerLab.title")}</h2>
      <div class="tl-sub">${a("tools.thermometerLab.subtitle")}</div>
    </div>
    <div class="tl-dash">
  ...
  `;const t={liquidType:"mercury",thermometerType:"liquid",bulbVolume:200,wallThickness:.5,liquidL0:3,liquidL100:13,resistanceR0:5,resistanceR100:6.2,thermistorR25:10,thermistorBeta:3500,bathTemp:25,thermometerTemp:25,bubbles:[],iceCubes:[],heatWaves:[],currentLength:5.5,currentResistance:5.3,currentThermistorR:10,lastTimestamp:0},i=460,r=300,f=460,q=300,T=n.querySelector("#tl-thermometerCanvas"),E=T.getContext("2d"),$=n.querySelector("#tl-graphCanvas"),I=$.getContext("2d"),_=window.devicePixelRatio||1;T.width=i*_,T.height=r*_,T.style.width=i+"px",T.style.height=r+"px",E.scale(_,_),$.width=f*_,$.height=q*_,$.style.width=f+"px",$.style.height=q+"px",I.scale(_,_);function F(){t.iceCubes=[];for(let e=0;e<4;e++)t.iceCubes.push({x:60+Math.random()*120,y:200+Math.random()*15,size:10+Math.random()*8,angle:Math.random()*Math.PI,speedX:(Math.random()-.5)*.15,speedY:(Math.random()-.5)*.15});t.bubbles=[];for(let e=0;e<15;e++)t.bubbles.push({x:60+Math.random()*120,y:190+Math.random()*70,r:1+Math.random()*3,speedY:.6+Math.random()*1.2,phase:Math.random()*Math.PI*2,wobbleSpeed:.08+Math.random()*.08});t.heatWaves=[];for(let e=0;e<6;e++)t.heatWaves.push({x:65+Math.random()*110,y:190+Math.random()*60,length:12+Math.random()*15,speedY:.4+Math.random()*.5,opacity:.1+Math.random()*.25})}function O(){if(t.thermometerType==="resistance"||t.thermometerType==="thermistor")return .35;const e=t.liquidType==="mercury"?1:8,o=.2+t.wallThickness*1.5,d=.4+t.bulbVolume*.003;return Math.max(.1,o*d*e*.15)}function y(e){t.bathTemp<=8&&t.iceCubes.forEach(o=>{o.x+=o.speedX,o.y+=o.speedY,(o.x<65||o.x>175)&&(o.speedX*=-1),(o.y<195||o.y>210)&&(o.speedY*=-1)}),t.bathTemp>35&&t.heatWaves.forEach(o=>{o.y-=o.speedY*(1+t.bathTemp/100),o.y<185&&(o.y=265,o.x=65+Math.random()*110)}),t.bathTemp>=80&&t.bubbles.forEach(o=>{o.y-=o.speedY*(1+(t.bathTemp-80)/40),o.phase+=o.wobbleSpeed,o.x+=Math.sin(o.phase)*.2,o.y<185&&(o.y=260+Math.random()*10,o.x=60+Math.random()*120)})}function L(e){let p=59,H=130,b=246;if(t.bathTemp<25){const x=t.bathTemp/25;p=Math.round(180-x*121),H=Math.round(210-x*80),b=Math.round(250-x*4)}else{const x=Math.min(1,(t.bathTemp-25)/125);p=Math.round(59+x*180),H=Math.round(130-x*100),b=Math.round(246-x*190)}if(t.bathTemp>40){const x=Math.min(1,(t.bathTemp-40)/100),G=Date.now()*.02;e.fillStyle="rgba(249, 115, 22, 0.85)",e.beginPath(),e.moveTo(55+130/2-20,284);for(let X=-20;X<=20;X+=8){const ee=x*(10+Math.sin(X*.3+G)*4);e.lineTo(55+130/2+X,284-ee)}e.lineTo(55+130/2+20,284),e.closePath(),e.fill()}e.strokeStyle="#9ca3af",e.lineWidth=2,e.fillStyle="#1f1f23",e.beginPath(),e.roundRect(55,180,130,100,[0,0,10,10]),e.fill();const S=e.createLinearGradient(55,190,55,280);S.addColorStop(0,`rgba(${p}, ${H}, ${b}, 0.3)`),S.addColorStop(1,`rgba(${p}, ${H}, ${b}, 0.6)`),e.fillStyle=S,e.beginPath(),e.moveTo(55,190);const C=t.bathTemp>=80?1.5:.4,v=t.bathTemp>=80?.08:.03,z=Date.now()*v;for(let x=55;x<=185;x+=5){const G=190+Math.sin(x*.15+z)*C;e.lineTo(x,G)}e.lineTo(185,280),e.lineTo(55,280),e.closePath(),e.fill(),t.bathTemp>35&&(e.strokeStyle=`rgba(239, 68, 68, ${Math.min(.4,(t.bathTemp-35)/120)})`,e.lineWidth=1,t.heatWaves.forEach(x=>{e.beginPath(),e.moveTo(x.x,x.y),e.quadraticCurveTo(x.x+Math.sin(x.y*.06)*4,x.y-x.length/2,x.x,x.y-x.length),e.stroke()})),t.bathTemp<=8&&(e.fillStyle="rgba(255, 255, 255, 0.6)",e.strokeStyle="rgba(147, 197, 253, 0.8)",e.lineWidth=.8,t.iceCubes.forEach(x=>{e.save(),e.translate(x.x,x.y),e.rotate(x.angle),e.beginPath(),e.roundRect(-x.size/2,-x.size/2,x.size,x.size,2),e.fill(),e.stroke(),e.restore()})),t.bathTemp>=80&&(e.fillStyle="rgba(255, 255, 255, 0.5)",e.strokeStyle="rgba(255, 255, 255, 0.7)",e.lineWidth=.5,t.bubbles.forEach(x=>{e.beginPath(),e.arc(x.x,x.y,x.r,0,Math.PI*2),e.fill(),e.stroke()})),e.strokeStyle="#4b5563",e.lineWidth=3,e.beginPath(),e.moveTo(54,180),e.lineTo(54,272),e.arcTo(54,281,65,281,8),e.lineTo(175,281),e.arcTo(186,281,186,272,8),e.lineTo(186,180),e.stroke()}function M(e){const S=e.createLinearGradient(114,20,126,20);S.addColorStop(0,"rgba(209, 213, 219, 0.85)"),S.addColorStop(.2,"rgba(255, 255, 255, 0.95)"),S.addColorStop(.5,"rgba(243, 244, 246, 0.2)"),S.addColorStop(.8,"rgba(255, 255, 255, 0.95)"),S.addColorStop(1,"rgba(156, 163, 175, 0.9)"),e.fillStyle=S,e.strokeStyle="rgba(156, 163, 175, 0.8)",e.lineWidth=1,e.beginPath(),e.moveTo(114,230),e.lineTo(114,25),e.arcTo(114,20,119,20,5),e.arcTo(126,20,126,25,5),e.lineTo(126,230),e.closePath(),e.fill(),e.stroke();const C=e.createRadialGradient(120-11*.2,250-11*.2,11*.1,120,250,11);C.addColorStop(0,"rgba(255, 255, 255, 0.7)"),C.addColorStop(1,"rgba(156, 163, 175, 0.3)"),e.fillStyle=C,e.strokeStyle="#6b7280",e.lineWidth=1.2,e.beginPath(),e.arc(120,250,12,0,Math.PI*2),e.fill(),e.stroke();const v=1.5;e.fillStyle="#111827",e.fillRect(120-v/2,28,v,202);const z=t.liquidType==="mercury",x=z?"#9ca3af":"#ef4444",G=z?"#f3f4f6":"#fecaca",X=210,ee=220,te=(X-40)/ee,Y=X-Math.min(ee,t.thermometerTemp)*te;e.fillStyle=x,e.fillRect(120-v/2,Y,v,230-Y),e.fillStyle=G,e.fillRect(120-v/6,Y,v/3,230-Y);const K=11*.9,U=e.createRadialGradient(120-K*.2,250-K*.2,K*.1,120,250,K);z?(U.addColorStop(0,"#f3f4f6"),U.addColorStop(.3,"#d1d5db"),U.addColorStop(.8,"#9ca3af"),U.addColorStop(1,"#4b5563")):(U.addColorStop(0,"#fee2e2"),U.addColorStop(.2,"#fca5a5"),U.addColorStop(.7,"#ef4444"),U.addColorStop(1,"#991b1b")),e.fillStyle=U,e.beginPath(),e.arc(120,250,K,0,Math.PI*2),e.fill(),e.strokeStyle="#4b5563",e.lineWidth=.5,e.font="6.5px Arial",e.fillStyle="#a1a1aa";for(let ie=0;ie<=200;ie+=50){const ce=X-ie*te;e.beginPath(),e.moveTo(114,ce),e.lineTo(118,ce),e.stroke(),e.fillText(`${ie}°C`,92,ce+2.5)}}function D(e){const p=e.createLinearGradient(116,20,124,20);p.addColorStop(0,"#9ca3af"),p.addColorStop(.3,"#f3f4f6"),p.addColorStop(.7,"#d1d5db"),p.addColorStop(1,"#6b7280"),e.fillStyle=p,e.strokeStyle="#4b5563",e.lineWidth=1,e.beginPath(),e.roundRect(116,20,8,225,[0,0,3,3]),e.fill(),e.stroke(),e.lineWidth=1.5,e.strokeStyle="#ef4444",e.beginPath(),e.moveTo(118,20),e.bezierCurveTo(110,5,200,30,245,60),e.stroke(),e.strokeStyle="#111827",e.beginPath(),e.moveTo(122,20),e.bezierCurveTo(130,8,210,45,245,75),e.stroke();const H=245,b=40,S=135,C=90;e.fillStyle="#1e1b4b",e.strokeStyle="#4f46e5",e.lineWidth=2.5,e.beginPath(),e.roundRect(H,b,S,C,10),e.fill(),e.stroke(),e.fillStyle="#022c22",e.beginPath(),e.roundRect(H+10,b+12,S-20,34,4),e.fill(),e.font='bold 14px "Courier New"',e.fillStyle="#34d399",e.textAlign="right",e.fillText(t.currentResistance.toFixed(2)+" Ω",H+S-16,b+34),e.font="6px Arial",e.fillStyle="#a7f3d0",e.textAlign="left",e.fillText("PLATINUM RTD METER",H+14,b+21)}function Q(e){e.strokeStyle="#111827",e.lineWidth=1.2,e.beginPath(),e.moveTo(118,20),e.lineTo(118,245),e.stroke(),e.beginPath(),e.moveTo(122,20),e.lineTo(122,245),e.stroke();const w=20,h=225,p=10,H=120-p/2,b=e.createLinearGradient(H,w,H+p,w);b.addColorStop(0,"rgba(229, 231, 235, 0.4)"),b.addColorStop(.3,"rgba(255, 255, 255, 0.8)"),b.addColorStop(.7,"rgba(243, 244, 246, 0.5)"),b.addColorStop(1,"rgba(209, 213, 219, 0.6)"),e.fillStyle=b,e.strokeStyle="rgba(156, 163, 175, 0.6)",e.lineWidth=.8,e.beginPath(),e.roundRect(H,w,p,h-w,[0,0,2,2]),e.fill(),e.stroke();const S=6,C=e.createRadialGradient(120-S*.2,245-S*.2,S*.1,120,245,S);C.addColorStop(0,"#4b5563"),C.addColorStop(.7,"#1f2937"),C.addColorStop(1,"#111827"),e.fillStyle=C,e.strokeStyle="#030712",e.lineWidth=1,e.beginPath(),e.arc(120,245,S,0,Math.PI*2),e.fill(),e.stroke(),e.lineWidth=1.5,e.strokeStyle="#ef4444",e.beginPath(),e.moveTo(118,20),e.bezierCurveTo(110,5,200,30,245,60),e.stroke(),e.strokeStyle="#111827",e.beginPath(),e.moveTo(122,20),e.bezierCurveTo(130,8,210,45,245,75),e.stroke();const v=245,z=40,x=135,G=90;e.fillStyle="#111827",e.strokeStyle="#10b981",e.lineWidth=2.5,e.beginPath(),e.roundRect(v,z,x,G,10),e.fill(),e.stroke(),e.fillStyle="#022c22",e.beginPath(),e.roundRect(v+10,z+12,x-20,34,4),e.fill(),e.font='bold 14px "Courier New"',e.fillStyle="#34d399",e.textAlign="right",e.fillText(t.currentThermistorR.toFixed(2)+" kΩ",v+x-16,z+34),e.font="6px Arial",e.fillStyle="#a7f3d0",e.textAlign="left",e.fillText("NTC THERMISTOR METER",v+14,z+21)}function W(e){e.strokeStyle="#4b5563",e.lineWidth=1.2,e.beginPath(),e.moveTo(50,30),e.lineTo(50,260),e.lineTo(430,260),e.stroke(),e.strokeStyle="#27272a",e.lineWidth=.5,e.fillStyle="#a1a1aa",e.font="7px Arial",e.textAlign="right",e.textBaseline="middle";const h=25,p=200;for(let v=0;v<=h;v+=5){const z=260-v/h*230;e.beginPath(),e.moveTo(50,z),e.lineTo(430,z),e.stroke(),e.fillText(`${v.toFixed(1)}`,44,z)}e.textAlign="center",e.textBaseline="top";for(let v=0;v<=p;v+=40){const z=50+v/p*380;e.beginPath(),e.moveTo(z,30),e.lineTo(z,260),e.stroke(),e.fillText(`${v}`,z,264)}e.save(),e.translate(24,30+230/2),e.rotate(-Math.PI/2),e.font="bold 8px Arial",e.fillStyle="#e4e4e7",e.fillText("Length of liquid column / cm",0,0),e.restore(),e.font="bold 8px Arial",e.fillText("temperature / °C",50+380/2,274);const H=260-t.liquidL0/h*230,b=260-(t.liquidL0+(t.liquidL100-t.liquidL0)/100*p)/h*230;e.strokeStyle="#ef4444",e.lineWidth=1.8,e.beginPath(),e.moveTo(50,H),e.lineTo(430,b),e.stroke();const S=t.thermometerTemp,C=t.currentLength;if(S>=0&&S<=p){const v=50+S/p*380,z=260-C/h*230;e.fillStyle="#ef4444",e.beginPath(),e.arc(v,z,4,0,Math.PI*2),e.fill()}}function N(e){e.strokeStyle="#4b5563",e.lineWidth=1.2,e.beginPath(),e.moveTo(50,30),e.lineTo(50,260),e.lineTo(430,260),e.stroke(),e.strokeStyle="#27272a",e.lineWidth=.5,e.fillStyle="#a1a1aa",e.font="7px Arial",e.textAlign="right",e.textBaseline="middle";const h=10,p=200;for(let v=0;v<=h;v+=2){const z=260-v/h*230;e.beginPath(),e.moveTo(50,z),e.lineTo(430,z),e.stroke(),e.fillText(`${v.toFixed(1)}`,44,z)}e.textAlign="center",e.textBaseline="top";for(let v=0;v<=p;v+=40){const z=50+v/p*380;e.beginPath(),e.moveTo(z,30),e.lineTo(z,260),e.stroke(),e.fillText(`${v}`,z,264)}e.save(),e.translate(24,30+230/2),e.rotate(-Math.PI/2),e.font="bold 8px Arial",e.fillStyle="#e4e4e7",e.fillText("Resistance of platinum / Ω",0,0),e.restore(),e.font="bold 8px Arial",e.fillText("temperature / °C",50+380/2,274);const H=260-t.resistanceR0/h*230,b=260-(t.resistanceR0+(t.resistanceR100-t.resistanceR0)/100*p)/h*230;e.strokeStyle="#4f46e5",e.lineWidth=1.8,e.beginPath(),e.moveTo(50,H),e.lineTo(430,b),e.stroke();const S=t.thermometerTemp,C=t.currentResistance;if(S>=0&&S<=p){const v=50+S/p*380,z=260-C/h*230;e.fillStyle="#4f46e5",e.beginPath(),e.arc(v,z,4,0,Math.PI*2),e.fill()}}function B(e){e.strokeStyle="#4b5563",e.lineWidth=1.2,e.beginPath(),e.moveTo(50,30),e.lineTo(50,260),e.lineTo(430,260),e.stroke(),e.strokeStyle="#27272a",e.lineWidth=.5,e.fillStyle="#a1a1aa",e.font="7px Arial",e.textAlign="right",e.textBaseline="middle";const h=20,p=200;for(let C=0;C<=h;C+=4){const v=260-C/h*230;e.beginPath(),e.moveTo(50,v),e.lineTo(430,v),e.stroke(),e.fillText(`${C.toFixed(1)}`,44,v)}e.textAlign="center",e.textBaseline="top";for(let C=0;C<=p;C+=40){const v=50+C/p*380;e.beginPath(),e.moveTo(v,30),e.lineTo(v,260),e.stroke(),e.fillText(`${C}`,v,264)}e.save(),e.translate(24,30+230/2),e.rotate(-Math.PI/2),e.font="bold 8px Arial",e.fillStyle="#e4e4e7",e.fillText("Resistance of thermistor / kΩ",0,0),e.restore(),e.font="bold 8px Arial",e.fillText("temperature / °C",50+380/2,274),e.strokeStyle="#10b981",e.lineWidth=1.8,e.beginPath();let H=!1;for(let C=0;C<=p;C+=2){const v=C+273.15,z=t.thermistorR25*Math.exp(t.thermistorBeta*(1/v-1/298.15)),x=50+C/p*380,G=260-Math.min(h,z)/h*230;G>=30&&G<=260&&(H?e.lineTo(x,G):(e.moveTo(x,G),H=!0))}e.stroke();const b=t.thermometerTemp,S=t.currentThermistorR;if(b>=0&&b<=p){const C=50+b/p*380,v=260-Math.min(h,S)/h*230;e.fillStyle="#10b981",e.beginPath(),e.arc(C,v,4,0,Math.PI*2),e.fill()}}function P(){E.clearRect(0,0,i,r),I.clearRect(0,0,f,q),L(E),t.thermometerType==="liquid"?(M(E),W(I)):t.thermometerType==="resistance"?(D(E),N(I)):(Q(E),B(I))}function j(e){n.querySelector("#tl-bath-temp-display").innerHTML=t.bathTemp.toFixed(1)+"°C",n.querySelector("#tl-val-bath-temp").innerHTML=t.bathTemp.toFixed(1)+" °C";const o=n.querySelector("#tl-bath-state");t.bathTemp<=0?o.textContent="Melting Ice Bath":t.bathTemp>=100?o.textContent="Boiling Water/Steam":t.bathTemp===150?o.textContent="Hot Cooking Oil":o.textContent="Liquid Water",n.querySelector("#tl-val-response-time").textContent=e.toFixed(2)+" s";const d=n.querySelector("#tl-alcohol-boiling-warning");if(t.thermometerType==="liquid"&&t.liquidType==="alcohol"&&t.bathTemp>=78?d.style.display="block":d.style.display="none",t.thermometerType==="liquid")n.querySelector("#tl-live-liquid-lt").textContent=t.currentLength.toFixed(2)+" cm",n.querySelector("#tl-live-liquid-sub-num").textContent=`${t.currentLength.toFixed(2)} - ${t.liquidL0.toFixed(1)}`,n.querySelector("#tl-live-liquid-sub-den").textContent=`${t.liquidL100.toFixed(1)} - ${t.liquidL0.toFixed(1)}`,n.querySelector("#tl-live-liquid-ans").textContent=t.thermometerTemp.toFixed(1)+"°C";else if(t.thermometerType==="resistance")n.querySelector("#tl-live-resistance-rt").textContent=t.currentResistance.toFixed(2)+" Ω",n.querySelector("#tl-live-resistance-sub-num").textContent=`${t.currentResistance.toFixed(2)} - ${t.resistanceR0.toFixed(1)}`,n.querySelector("#tl-live-resistance-sub-den").textContent=`${t.resistanceR100.toFixed(1)} - ${t.resistanceR0.toFixed(1)}`,n.querySelector("#tl-live-resistance-ans").textContent=t.thermometerTemp.toFixed(1)+"°C";else{n.querySelector("#tl-live-thermistor-rt").textContent=t.currentThermistorR.toFixed(2)+" kΩ";const u=t.thermometerTemp+273.15;n.querySelector("#tl-live-thermistor-calc-tk").textContent=u.toFixed(2),n.querySelector("#tl-live-thermistor-ans").textContent=t.thermometerTemp.toFixed(1)+"°C"}}let V=null;function g(e){t.lastTimestamp||(t.lastTimestamp=e);const o=(e-t.lastTimestamp)/1e3;t.lastTimestamp=e;const d=Math.min(o,.1),u=O(),w=d/u*(t.bathTemp-t.thermometerTemp);t.thermometerTemp+=w,t.currentLength=t.liquidL0+(t.liquidL100-t.liquidL0)/100*t.thermometerTemp,t.currentResistance=t.resistanceR0+(t.resistanceR100-t.resistanceR0)/100*t.thermometerTemp;const h=t.thermometerTemp+273.15;t.currentThermistorR=t.thermistorR25*Math.exp(t.thermistorBeta*(1/h-1/298.15)),y(),P(),j(u),V=requestAnimationFrame(g)}function k(){const o=((parseFloat(n.querySelector("#tl-input-q10a-cm").value)||0)- -1.5)/106.5*100;n.querySelector("#tl-ans-q10a").textContent=o.toFixed(1)+" °C"}function A(){const o=(parseFloat(n.querySelector("#tl-input-q10b-t").value)||0)/100*106.5-1.5;n.querySelector("#tl-ans-q10b").textContent=o.toFixed(2)+" °C"}function R(){const e=parseFloat(n.querySelector("#tl-input-q11-r").value)||0,o=(e-t.resistanceR0)/(t.resistanceR100-t.resistanceR0)*100;n.querySelector("#tl-formula-q11-rsub").textContent=e.toFixed(1),n.querySelector("#tl-ans-q11").textContent=o.toFixed(1)+" °C"}function c(){k(),A(),R()}function l(e,o){n.querySelector("#"+e).addEventListener("click",()=>{t.bathTemp=o,n.querySelector("#tl-bath-temp-slider").value=o,c()})}function m(){n.querySelectorAll(".tl-tab-btn").forEach(o=>{o.addEventListener("click",()=>{n.querySelectorAll(".tl-tab-btn").forEach(u=>u.classList.remove("active")),n.querySelectorAll(".tl-tab-content").forEach(u=>u.classList.remove("active")),o.classList.add("active");const d=o.getAttribute("data-tl-tab");n.querySelector("#tl-tab-"+d).classList.add("active"),t.thermometerType=d,c()})}),n.querySelector("#tl-bath-temp-slider").addEventListener("input",o=>{t.bathTemp=parseFloat(o.target.value),c()}),l("tl-btn-preset-ice",0),l("tl-btn-preset-room",25),l("tl-btn-preset-steam",100),l("tl-btn-preset-oil",150),n.querySelector("#tl-card-mercury").addEventListener("click",()=>{t.liquidType="mercury",n.querySelector("#tl-card-mercury").className="tl-radio-card active-mercury",n.querySelector("#tl-card-alcohol").className="tl-radio-card",c()}),n.querySelector("#tl-card-alcohol").addEventListener("click",()=>{t.liquidType="alcohol",n.querySelector("#tl-card-mercury").className="tl-radio-card",n.querySelector("#tl-card-alcohol").className="tl-radio-card active-alcohol",c()}),n.querySelector("#tl-slider-bulb-vol").addEventListener("input",o=>{t.bulbVolume=parseFloat(o.target.value),n.querySelector("#tl-val-bulb-vol").textContent=t.bulbVolume+" mm³"}),n.querySelector("#tl-slider-wall-thick").addEventListener("input",o=>{t.wallThickness=parseFloat(o.target.value),n.querySelector("#tl-val-wall-thickness").textContent=t.wallThickness.toFixed(1)+" mm"}),n.querySelector("#tl-slider-liquid-l0").addEventListener("input",o=>{t.liquidL0=parseFloat(o.target.value),n.querySelector("#tl-val-liquid-l0").textContent=t.liquidL0.toFixed(1)+" cm",c()}),n.querySelector("#tl-slider-liquid-l100").addEventListener("input",o=>{t.liquidL100=parseFloat(o.target.value),n.querySelector("#tl-val-liquid-l100").textContent=t.liquidL100.toFixed(1)+" cm",c()}),n.querySelector("#tl-slider-resistance-r0").addEventListener("input",o=>{t.resistanceR0=parseFloat(o.target.value),n.querySelector("#tl-val-resistance-r0").textContent=t.resistanceR0.toFixed(1)+" Ω",n.querySelector("#tl-spec-resistance-r0").textContent=t.resistanceR0.toFixed(1)+" Ω",c()}),n.querySelector("#tl-slider-resistance-r100").addEventListener("input",o=>{t.resistanceR100=parseFloat(o.target.value),n.querySelector("#tl-val-resistance-r100").textContent=t.resistanceR100.toFixed(1)+" Ω",n.querySelector("#tl-spec-resistance-r100").textContent=t.resistanceR100.toFixed(1)+" Ω",c()}),n.querySelector("#tl-slider-thermistor-r25").addEventListener("input",o=>{t.thermistorR25=parseFloat(o.target.value),n.querySelector("#tl-val-thermistor-r25").textContent=t.thermistorR25.toFixed(1)+" kΩ",n.querySelector("#tl-spec-thermistor-r25").textContent=t.thermistorR25.toFixed(1)+" kΩ",c()}),n.querySelector("#tl-slider-thermistor-beta").addEventListener("input",o=>{t.thermistorBeta=parseFloat(o.target.value),n.querySelector("#tl-val-thermistor-beta").textContent=t.thermistorBeta+" K",n.querySelector("#tl-spec-thermistor-beta").textContent=t.thermistorBeta+" K",c()}),n.querySelector("#tl-btn-solve-q10a").addEventListener("click",()=>{n.querySelector("#tl-btn-solve-q10a").classList.add("active"),n.querySelector("#tl-btn-solve-q10b").classList.remove("active"),n.querySelector("#tl-pane-q10a").classList.add("active"),n.querySelector("#tl-pane-q10b").classList.remove("active")}),n.querySelector("#tl-btn-solve-q10b").addEventListener("click",()=>{n.querySelector("#tl-btn-solve-q10a").classList.remove("active"),n.querySelector("#tl-btn-solve-q10b").classList.add("active"),n.querySelector("#tl-pane-q10a").classList.remove("active"),n.querySelector("#tl-pane-q10b").classList.add("active")}),n.querySelector("#tl-input-q10a-cm").addEventListener("input",k),n.querySelector("#tl-input-q10b-t").addEventListener("input",A),n.querySelector("#tl-input-q11-r").addEventListener("input",R)}return F(),m(),c(),V=requestAnimationFrame(g),n._thermometerLabCleanup=()=>{V&&cancelAnimationFrame(V)},n}function oe(){return J()==="zh-Hant"?"zhHant":"en"}async function Ce(a,n){const t=`./${a}/${n}`;try{return(await fetch(t,{method:"HEAD"})).ok}catch{return!1}}async function at(a){return Ce("notes",a)}function nt(a){let n="topics",t=0,i="all",r=!1,f=null,q={main:null};function T(){if(q.main){if(n==="topics"?q.main.innerHTML=$():n==="notes"?q.main.innerHTML=_():n==="tools"?q.main.innerHTML=O():n==="worksheets"?q.main.innerHTML=Se(s):n==="flashcards"?q.main.innerHTML=M():n==="summary"&&(q.main.innerHTML=B()),n==="notes"&&F(),n==="tools"&&L(),n==="worksheets"){const g=we.filter(A=>["liquidThermometer","faultyThermometer","resistanceThermometer","thermistor"].includes(A.topic));Te(a,g,s,oe);const k=a.querySelector("[data-ws-paper] h3");k&&(k.textContent=s("worksheets.paperTitleHeat"))}n==="flashcards"&&N(),n==="summary"&&P()}}function E(){f==null||f.destroy(),f=qe(a,{subtitleKey:"strand.heat.subtitle",activeSection:n,onSection:g=>{n=g,f.updateSection(n),T()},onLang:()=>E()}),q.main=f.main,f.updateSection(n),T()}function $(){const g=[["liquidThermometer","topic.liquidThermometer"],["faultyThermometer","topic.faultyThermometer"],["resistanceThermometer","topic.resistanceThermometer"],["thermistor","topic.thermistor"]];return`
      <section class="panel panel--topic-hub">
        <h2>${s("topics.title")}</h2>
        <p class="lead">${s("topics.intro")}</p>
        <div class="grid cols-2 topic-hub-grid">
          ${g.map(([k,A])=>`
            <div class="card">
              <h3>${s(A)}</h3>
              <button class="btn primary" type="button" data-go-tool="thermometerLab">${s("topic.openTool")}</button>
            </div>`).join("")}
        </div>
      </section>`}function I(g){g.target.closest("[data-go-tool]")&&(n="tools",f.updateSection(n),T())}function _(){const g=[{key:"liquidThermometer",fileEn:"liquid-thermometer-en.pdf",fileZh:"liquid-thermometer-zhHant.pdf"},{key:"faultyThermometer",fileEn:"faulty-thermometer-en.pdf",fileZh:"faulty-thermometer-zhHant.pdf"},{key:"resistanceThermometer",fileEn:"resistance-thermometer-en.pdf",fileZh:"resistance-thermometer-zhHant.pdf"},{key:"thermistor",fileEn:"thermistor-en.pdf",fileZh:"thermistor-zhHant.pdf"}];return`
      <section class="panel">
        <h2>${s("notes.title")}</h2>
        <p class="lead">${s("notes.intro")}</p>
        <p class="lead">${s("notes.embedHint")}</p>
        <div class="grid cols-2" data-notes-grid>
          ${g.map(k=>{const A=s(`notes.card.${k.key}`);return`
            <div class="card" data-note-card="${k.key}">
              <h3>${A}</h3>
              <div data-note-body></div>
            </div>`}).join("")}
        </div>
      </section>`}async function F(){const g=[{key:"liquidThermometer",fileEn:"liquid-thermometer-en.pdf",fileZh:"liquid-thermometer-zhHant.pdf"},{key:"faultyThermometer",fileEn:"faulty-thermometer-en.pdf",fileZh:"faulty-thermometer-zhHant.pdf"},{key:"resistanceThermometer",fileEn:"resistance-thermometer-en.pdf",fileZh:"resistance-thermometer-zhHant.pdf"},{key:"thermistor",fileEn:"thermistor-en.pdf",fileZh:"thermistor-zhHant.pdf"}],k=oe();for(const A of g){const R=a.querySelector(`[data-note-card="${A.key}"]`);if(!R)continue;const c=R.querySelector("[data-note-body]"),l=k==="zhHant"?A.fileZh:A.fileEn,m=await at(l),e=`./notes/${l}`;m?c.innerHTML=`
          <iframe class="notes-grid" title="${s(`notes.card.${A.key}`)}" src="${e}"></iframe>
          <p style="margin-top:8px"><a href="${e}" target="_blank" rel="noopener">${s("notes.openPdf")}</a></p>`:c.innerHTML=`<p class="lead">${s("notes.missing")}</p>`}}function O(){return`
      <section class="panel panel--tools">
        <h2>${s("tools.title")}</h2>
        <div class="tools-layout">
          <div class="tool-stage" data-tool-stage style="width: 100%"></div>
        </div>
      </section>`}let y=null;function L(){const g=a.querySelector("[data-tool-stage]");g&&(g.innerHTML="",y&&y._thermometerLabCleanup&&y._thermometerLabCleanup(),y=tt(s),g.appendChild(y))}function M(){return`
      <section class="panel">
        <h2>${s("flashcards.title")}</h2>
        <p class="lead">${s("flashcards.intro")}</p>
        <div class="control" style="margin-bottom:10px">
          ...
          <select data-flash-deck>
            <option value="all">${s("flashcards.all")}</option>
            <option value="liquidThermometer">${s("topic.liquidThermometer")}</option>
            <option value="faultyThermometer">${s("topic.faultyThermometer")}</option>
            <option value="resistanceThermometer">${s("topic.resistanceThermometer")}</option>
            <option value="thermistor">${s("topic.thermistor")}</option>
          </select>
        </div>
        <div class="flashcard-box">
          <div class="flashcard-surface" data-flip-card>
      ...
      </section>`}function D(){return xe.filter(g=>["liquidThermometer","faultyThermometer","resistanceThermometer","thermistor"].includes(g.topic))}function Q(){const g=D();return i==="all"?g:g.filter(k=>k.topic===i)}function W(){const g=Q(),k=a.querySelector("[data-flash-progress]");if(!g.length){t=0;const o=a.querySelector("[data-flash-front]"),d=a.querySelector("[data-flip-card]");o&&(o.textContent=""),d&&d.classList.remove("flashcard-surface--image"),k&&(k.hidden=!0,k.textContent="");return}t=Math.max(0,Math.min(t,g.length-1));const A=g[t],R=a.querySelector("[data-flash-front]"),c=a.querySelector("[data-flip-card]"),l=a.querySelector("[data-flash-label]");if(!R||!c)return;k&&(k.hidden=!1,k.textContent=s("flashcards.progress").replace("{current}",String(t+1)).replace("{total}",String(g.length))),c.classList.remove("flashcard-surface--image");const m=oe(),e=A[m]||A.en;l&&(l.hidden=!1,l.textContent=s(r?"flashcards.answer":"flashcards.question")),R.textContent=r?e.a:e.q}function N(){const g=a.querySelector("[data-flash-deck]");g.value=i,g.addEventListener("change",()=>{i=g.value,t=0,r=!1,W()}),a.querySelector("[data-flip-card]").addEventListener("click",()=>{r=!r,W()}),a.querySelector("[data-flash-flip]").addEventListener("click",k=>{k.stopPropagation(),r=!r,W()}),a.querySelector("[data-flash-prev]").addEventListener("click",()=>{const k=Q();t=(t-1+k.length)%k.length,r=!1,W()}),a.querySelector("[data-flash-next]").addEventListener("click",()=>{const k=Q();t=(t+1)%k.length,r=!1,W()}),a.querySelector("[data-flash-shuf]").addEventListener("click",()=>{const k=Q();t=Math.floor(Math.random()*k.length),r=!1,W()}),W()}function B(){const g=[{key:"liquidThermometer",fileEn:"liquid-thermometer-en.pdf",fileZh:"liquid-thermometer-zhHant.pdf"},{key:"faultyThermometer",fileEn:"faulty-thermometer-en.pdf",fileZh:"faulty-thermometer-zhHant.pdf"},{key:"resistanceThermometer",fileEn:"resistance-thermometer-en.pdf",fileZh:"resistance-thermometer-zhHant.pdf"},{key:"thermistor",fileEn:"thermistor-en.pdf",fileZh:"thermistor-zhHant.pdf"}];return`
      <section class="panel">
        <h2>${s("summary.title")}</h2>
        <p class="lead">${s("summary.intro")}</p>
        <p class="lead">${s("notes.embedHint")}</p>
        <div class="grid cols-2" data-summary-grid>
          ${g.map(k=>{const A=s(`summary.item.${k.key}`);return`
            <div class="card" data-summary-card="${k.key}">
              <h3>${A}</h3>
              <div data-summary-body></div>
            </div>`}).join("")}
        </div>
      </section>`}async function P(){const g=[{key:"liquidThermometer",fileEn:"liquid-thermometer-en.pdf",fileZh:"liquid-thermometer-zhHant.pdf"},{key:"faultyThermometer",fileEn:"faulty-thermometer-en.pdf",fileZh:"faulty-thermometer-zhHant.pdf"},{key:"resistanceThermometer",fileEn:"resistance-thermometer-en.pdf",fileZh:"resistance-thermometer-zhHant.pdf"},{key:"thermistor",fileEn:"thermistor-en.pdf",fileZh:"thermistor-zhHant.pdf"}],k=oe();for(const A of g){const R=a.querySelector(`[data-summary-card="${A.key}"]`);if(!R)continue;const c=R.querySelector("[data-summary-body]"),l=k==="zhHant"?A.fileZh:A.fileEn,m=await Ce("summary-pdfs",l),e=`./summary-pdfs/${l}`;m?c.innerHTML=`
          <iframe class="notes-grid" title="${s(`summary.item.${A.key}`)}" src="${e}"></iframe>
          <p style="margin-top:8px"><a href="${e}" target="_blank" rel="noopener">${s("summary.download")}</a></p>`:c.innerHTML=`<p class="lead">${s("summary.missing")}</p>`}}const j=()=>E(),V=g=>I(g);return window.addEventListener("s3phy:lang",j),a.addEventListener("click",V),E(),()=>{window.removeEventListener("s3phy:lang",j),a.removeEventListener("click",V),y&&y._thermometerLabCleanup&&y._thermometerLabCleanup(),f==null||f.destroy()}}const pe={en:["F = ma","E = mc²","p = mv","v = u + at","s = ut + ½at²","v² = u² + 2as","W = Fs cos θ","P = W / t","Eₖ = ½mv²","Eₚ = mgh","P = Fv","λ = h / p","E = hf","c = fλ","v = fλ","n₁ sin θ₁ = n₂ sin θ₂","η = sin i / sin r","1/f = 1/u + 1/v","m = v/u","V = IR","P = IV","ρ = m/V","Reflection","Refraction","Total internal reflection","Convex lens","Electromagnetic waves"],zhHant:["F = ma","E = mc²","p = mv","v = u + at","W = Fs cos θ","Eₖ = ½mv²","Eₚ = mgh","λ = h / p","E = hf","c = fλ","n₁ sin θ₁ = n₂ sin θ₂","1/f = 1/u + 1/v","V = IR","ρ = m/V","反射","折射","全內反射","凸透鏡","凹透鏡","電磁波"]};ze();const fe="s3phy_splash_seen",_e={optics:Ze,heat:nt};let ae=null,ne=null;function it(){const a=location.hash.match(/^#\/(\w+)/),n=a==null?void 0:a[1];return n&&Object.prototype.hasOwnProperty.call(_e,n)?n:null}function rt(){const a=`${location.pathname}${location.search}`;history.replaceState(null,"",a)}function Me(a){ae==null||ae(),ae=null,ne==null||ne(),ne=null;const n=document.getElementById("app");if(!n)return;if(n.innerHTML="",!a){location.hash&&rt(),ne=$e(n);return}const t=`#/${a}`;if(location.hash!==t){location.hash=t;return}ae=_e[a](n)??null}function Re(){Me(it())}function ot(){return"./images/uniplus-logo.png"}function st(){const a=document.createElement("div");return a.id="splash",a.className="splash",a.setAttribute("role","dialog"),a.setAttribute("aria-modal","true"),a.setAttribute("aria-label","Uni+"),a.innerHTML=`
    <div class="splash-danmaku" id="splashDanmaku" aria-hidden="true"></div>
    <div class="splash-stage">
      <div class="splash-logo-wrap" id="splashLogoWrap">
        <img class="splash-logo-img" src="${ot()}" alt="Uni+ logo" decoding="async" width="520" height="120" />
      </div>
  ...
  `,a}function lt(a,n){const t=[...pe.en,...pe.zhHant],i=()=>{if(!a.parentElement)return;const r=document.createElement("div");r.className="splash-danmaku-item",r.textContent=t[Math.floor(Math.random()*t.length)];const f=Math.random()*85+5,q=Math.random()*.8+.9,T=Math.random()*12+10;r.style.top=`${f}%`,r.style.fontSize=`${q}rem`,r.style.animationDuration=`${T}s`,n.appendChild(r),r.addEventListener("animationend",()=>r.remove()),window.setTimeout(i,1500+Math.random()*1e3)};for(let r=0;r<4;r+=1)window.setTimeout(i,r*1e3)}function ct(a){const n=a.querySelector("#splashLogoWrap"),t=a.querySelector("#splashDanmaku"),i=a.querySelector("#splashStartWrap");window.setTimeout(()=>n==null?void 0:n.classList.add("splash-logo-wrap--in"),800),window.setTimeout(()=>{t==null||t.classList.add("splash-danmaku--visible"),i==null||i.classList.add("splash-start-wrap--in"),t&&lt(a,t)},1600)}function ge(){Re()}function dt(){let a=!1;try{a=sessionStorage.getItem(fe)==="1"}catch{}if(a){ge();return}const n=st();document.body.insertBefore(n,document.body.firstChild),ct(n);const t=n.querySelector("#splashStart"),i=()=>{n.remove(),ge()};t.addEventListener("click",()=>{try{sessionStorage.setItem(fe,"1")}catch{}n.classList.add("splash--hide"),n.addEventListener("transitionend",i,{once:!0}),window.setTimeout(i,480)})}window.addEventListener("hashchange",Re);window.addEventListener("s3phy:strand",a=>{Me(a.detail??null)});dt();
