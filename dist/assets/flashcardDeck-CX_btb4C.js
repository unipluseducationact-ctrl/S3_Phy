import{t as r,g as M,s as O,_ as z}from"./index-BfmbmKAK.js";function F(){return M()==="zh-Hant"?"zhHant":"en"}async function R(t,n){const o=`./${t}/${n}`;try{return(await fetch(o,{method:"HEAD"})).ok}catch{return!1}}async function U(t){return R("notes",t)}function W(t){return`${String(t).split("#")[0]}#view=FitH&toolbar=0&navpanes=0&scrollbar=0`}function G(t,n,o){const l=W(n);return`
    ...
    <p class="note-preview-link"><a href="${n}" target="_blank" rel="noopener">${o}</a></p>`}async function pt(t,n){const o=F();await Promise.all(n.map(async l=>{const d=t.querySelector(`[data-note-card="${l.key}"]`);if(!d)return;const e=d.querySelector("[data-note-body]"),i=o==="zhHant"?l.fileZh:l.fileEn,a=await U(i),y=`./notes/${i}`;a?e.innerHTML=G(r(`notes.card.${l.key}`),y,r("notes.openPdf")):e.innerHTML=`<p class="lead">${r("notes.missing")}</p>
          ...
          <p style="margin-top:8px"><a href="${u}" target="_blank" rel="noopener">${r("summary.viewImage")}</a></p>`:a.innerHTML=`<p class="lead">${r("summary.missing")}</p>`;return}const y=l==="zhHant"?e.fileZh:e.fileEn,m=await R("summary-pdfs",y),h=`./summary-pdfs/${y}`;m?a.innerHTML=G(r(`summary.item.${e.key}`),h,r("summary.download")):a.innerHTML=`<p class="lead">${r("summary.missing")}</p>`}))}const K=["topics","notes","tools","worksheets","flashcards","summary"],j={topics:"nav.topics",notes:"nav.notes",tools:"nav.tools",worksheets:"nav.worksheets",flashcards:"nav.flashcards",summary:"nav.summary"};function gt(t,{subtitleKey:n,activeSection:o,onSection:l,onLang:d}){t.innerHTML=`
