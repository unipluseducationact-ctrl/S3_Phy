(() => {
  ...
  function normalizeFillAnswer(s) {
    return String(s || "").trim().toLowerCase().replace(/×/g, "x").replace(/⁻¹/g, "^-1").replace(/⁻/g, "-").replace(/\^\s*\(\s*-\s*1\s*\)/g, "^-1").replace(/\^\s*\(\s*(\d+)\s*\)/g, "^$1").replace(/ms(\^-1|-1)/g, "m s$1").replace(/m s-1/g, "m s^-1").replace(/\s+/g, " ").replace(/[.,;]/g, "");
  }
