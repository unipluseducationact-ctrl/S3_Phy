...
const UI = {
  en: {
    appSubtitle: "F.3 Heat Final Exam · English / 繁體中文 UI",
    ...
    revBandFair: "Mixed performance: re-read heat notes for weaker topics (SHC, latent heat, heat transfer), then regenerate.",
    revBandLow:
      "Several concepts need consolidation. Review specific heat capacity, latent heat, heating curves, and heat transfer before the next round.",
  ...
  zh: {
    ...
    revBandFair: "表现参差：请重温比热容、潜热与热传递笔记后再生成题目。",
    revBandLow: "多个概念仍需巩固。请先温习比热容、潜热、加热曲线与热传递。",
  ...
  "zh-Hant": {
    ...
    revBandFair: "表現參差：請重溫比熱容、潛熱與熱傳遞筆記後再產生題目。",
    revBandLow: "多個概念仍需鞏固。請先溫習比熱容、潛熱、加熱曲線與熱傳遞。",
...
export function initQuiz() {
  ...
  function appendStem(wrap, q) {
    ...
    if (fmt === "fill") {
      ...
      stem.className =
        "split-text-target font-headline-lg-mobile text-headline-lg-mobile text-on-surface mb-4 leading-relaxed whitespace-pre-line";
      setStemText(stem, String(q.stem || "").replace(/\n\n(\([a-d]\))/i, " $1"));
    ...
    if (statements) {
      ...
      introEl.className =
        "split-text-target font-headline-lg-mobile text-headline-lg-mobile text-on-surface mb-3 leading-tight whitespace-pre-line";
      setStemText(introEl, intro);
      wrap.appendChild(introEl);

      const list = document.createElement("div");
      list.className = "statement-list space-y-2 mb-4";
      statements.forEach((stmt) => {
        const line = document.createElement("p");
        line.className =
          "text-body-md text-on-surface leading-relaxed border-l-2 border-outline-variant/30 pl-3";
        setStemText(line, stmt);
  ...
  function initMeta() {
    ...
    if (els.formatChecks) {
      const formats = availableFormatFilters(QUIZ_ITEMS);
      els.formatChecks.innerHTML = formats
        .map((fmt) => {
  ...
  function renderQuiz() {
    ...
    lastQuestions.forEach((q, idx) => {
      ...
      if (fmt === "fill" && countFillBlanks(q) > 0) {
        ...
        let tableHtml =
          '<table class="quiz-option-table w-full text-left text-body-sm border-collapse"><thead><tr><th class="w-14"></th>';
        headers.forEach((h) => {
          tableHtml += `<th>${escHtml(h)}</th>`;
        });
        tableHtml += "</tr></thead><tbody>";
        q.optionTable.rows.forEach((row) => {
          tableHtml += `<tr data-key="${escHtml(row.key)}"><td class="align-middle"><button type="button" class="quiz-option-table-key" data-key="${escHtml(row.key)}" ${st.solved ? "disabled" : ""}>${escHtml(row.key)}</button></td>`;
          (row.cells || []).forEach((cell) => {
            tableHtml += `<td>${escHtml(formatQuizText(cell))}</td>`;
          });
          tableHtml += "</tr>";
        });
        ...
        tableWrap.querySelectorAll(".quiz-option-table-key").forEach((btnOpt) => {
          optionButtons.push(btnOpt);
          if (!st.solved) {
            btnOpt.addEventListener("click", () => {
              tableWrap.querySelectorAll(".quiz-option-table-key").forEach((b) => {
                b.classList.remove("is-selected");
              });
              tableWrap.querySelectorAll("tr").forEach((tr) => tr.classList.remove("is-selected"));
              btnOpt.classList.add("is-selected");
              btnOpt.closest("tr")?.classList.add("is-selected");
              const state = attemptMap.get(q.id) || { wrong: 0, solved: false, selected: null };
              state.selected = btnOpt.dataset.key;
              attemptMap.set(q.id, state);
              wrap.dataset.selected = btnOpt.dataset.key;
            });
          } else if (btnOpt.dataset.key === q.answer) {
            btnOpt.classList.add("is-selected", "is-correct");
            btnOpt.closest("tr")?.classList.add("is-selected", "is-correct");
          }
        });

        if (st.selected && !st.solved) {
          const sel = tableWrap.querySelector(`.quiz-option-table-key[data-key="${st.selected}"]`);
          sel?.classList.add("is-selected");
          sel?.closest("tr")?.classList.add("is-selected");
        }
      ...
      const showModelAnswer = () => {
        ...
        fb.innerHTML = `<strong>${escHtml(t("modelPrefix"))}</strong> ${escHtml(formatQuizText(ma.en))}${
      ...
      btn.addEventListener("click", () => {
        ...
        if (state.wrong === 1) {
          ...
          fb.innerHTML = `<strong>${escHtml(t("hintPrefix"))}</strong> ${escHtml(formatQuizText(q.hint || ""))}`;
        } else {
          state.solved = true;
          attemptMap.set(q.id, state);
          showModelAnswer();
