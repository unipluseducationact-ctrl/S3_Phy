#!/usr/bin/env python3
"""Sync Convex lens set 3 bilingual flashcard images into public/ + manifest."""
...
DEFAULT_SRC = Path(
    r"C:\Users\UniplusUser02\Desktop\PHYS\S3\Optics\Flashcards\Convex lens set 3"
)
OUT_DIR = ROOT / "public" / "flashcards" / "convex"
OUT_JSON = ROOT / "src" / "data" / "flashcards-convex.json"
REPORT = ROOT / "scripts" / "flashcards-convex-sync-report.md"
...
CARD_TITLES = {
    ...
    "RayRule2": "Ray Rule 2",
    "RayRule3": "Ray Rule 3",
    "ObjectInfinity": "Object at Infinity",
    "ObjectBeyond2F": "Object beyond 2F",
    "ObjectAt2F": "Object at 2F",
    "ObjectF2F": "Object between F-2F",
    "ObjectLensF": "Object between Lens-F",
    "ObjectAtF": "Object at F",
...
PATTERN = re.compile(
    r"^(EN|ZH)-CL-(.+?)-(Front|Back)(?:-v\d+)?\.(png|jpe?g)$",
    re.IGNORECASE,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sync Convex lens set 3 bilingual flashcard images"
    )
    ...
    report: list[str] = ["# Convex lens set 3 bilingual flashcard image sync\n\n"]
    if all_skipped:
    ...
    for num, card_key in enumerate(CARD_ORDER, start=1):
        ...
        for lang_key, json_key in (("en", "en"), ("zh", "zhHant")):
            ...
            front_src = g["front"]
            back_src = g.get("back")
            front_rel = f"./flashcards/convex/{prefix}-{lang_key}-front.webp"
            back_rel = f"./flashcards/convex/{prefix}-{lang_key}-back.webp"
