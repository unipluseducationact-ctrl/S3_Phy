#!/usr/bin/env python3
"""Sync Convex lens set 2 flashcard images into public/ + manifest."""
...
DEFAULT_SRC = Path(
    r"C:\Users\UniplusUser02\Desktop\PHYS\S3\Optics\Flashcards\Convex lens set 2"
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync Convex lens set 2 flashcard images")
    ...
    report: list[str] = ["# Convex lens set 2 flashcard image sync\n\n"]
    if unmapped:
    ...
    for num, title in enumerate(CARD_ORDER, start=1):
        ...
        back_src = g.get("back")
        prefix = f"{num:02d}"
        front_rel = f"./flashcards/convex/{prefix}-front.webp"
        back_rel = f"./flashcards/convex/{prefix}-back.webp"
