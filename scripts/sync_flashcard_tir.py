#!/usr/bin/env python3
"""Sync Total internal reflection set 2 flashcard images into public/ + manifest."""
...
DEFAULT_SRC = Path(
    r"C:\Users\UniplusUser02\Desktop\PHYS\S3\Optics\Flashcards\Total internal reflection set 2"
)
...
PATTERN = re.compile(
    r"^TIR Flashcard (Front|Back) \d+ - (.+?)(?:\s*\((.+?)\))?\s*\.(png|jpe?g)$",
    re.IGNORECASE,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync TIR set 2 flashcard images")
    ...
    report: list[str] = ["# TIR set 2 flashcard image sync\n\n"]
    if unmapped:
    ...
    for num, title in enumerate(CARD_ORDER, start=1):
        ...
        prefix = f"{num:02d}"
        front_rel = f"./flashcards/tir/{prefix}-front.webp"
        back_rel = f"./flashcards/tir/{prefix}-back.webp"
