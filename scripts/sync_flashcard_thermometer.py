#!/usr/bin/env python3
"""Sync Thermometer bilingual flashcard images into public/ + manifest."""
...
DEFAULT_SRC = Path(
    r"C:\Users\UniplusUser02\Desktop\PHYS\S3\Heat\Flashcards\Thermometer"
)
OUT_DIR = ROOT / "public" / "flashcards" / "thermometer"
OUT_JSON = ROOT / "src" / "data" / "flashcards-thermometer.json"
REPORT = ROOT / "scripts" / "flashcards-thermometer-sync-report.md"

MAX_WIDTH = 1400
WEBP_QUALITY = 82

CARD_TITLES = {
    "01": "What is Temperature?",
    "02": "Celsius Scale & Fixed Points",
    "03": "Calibration of Unmarked Thermometer",
    "04": "Liquid-in-Glass Thermometer",
    "05": "Mercury vs Alcohol Thermometer",
    "06": "Types of Thermometers",
    "07": "Temperature-Dependent Properties",
    "08": "Sensitivity of Thermometer",
    "09": "Calibration Calculation",
    "10": "Faulty Thermometer Correction",
}
...
PATTERN = re.compile(
    r"^(EN|ZH)-Thermometer-(\d{2})-.+-(Front|Back)(?:-v\d+)?(?: \(\d+\))?\.(png|jpe?g)$",
    re.IGNORECASE,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync Thermometer bilingual flashcard images")
    ...
    report: list[str] = ["# Thermometer bilingual flashcard image sync\n\n"]
    if all_skipped:
    ...
    for num in CARD_ORDER:
        ...
        for lang_key, json_key in (("en", "en"), ("zh", "zhHant")):
            ...
            front_src = g["front"]
            back_src = g.get("back")
            front_rel = f"./flashcards/thermometer/{num}-{lang_key}-front.webp"
            back_rel = f"./flashcards/thermometer/{num}-{lang_key}-back.webp"
