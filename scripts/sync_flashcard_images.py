#!/usr/bin/env python3
"""Sync Reflection set 3 bilingual flashcard images into public/ + manifest."""
...
DEFAULT_SRC = Path(
    r"C:\Users\UniplusUser02\Desktop\PHYS\S3\Optics\Flashcards\Reflection set 3"
)
...
CARD_TITLES = {
    "LawOfReflection": "Law of Reflection",
    "AngleDefinitions": "Angle Definitions",
    "RayDiagramConstruction": "Ray Diagram Construction",
    "ImageProperties": "Image Properties",
    "ObjectImageDistance": "Object-Image Distance",
    "VisibilityCondition": "Visibility Condition",
    "Reversibility": "Reversibility of Light",
...
PATTERN = re.compile(
    r"^(EN|ZH)-Reflection-(.+?)-(Front|Back)(?:-v\d+)?\.(png|jpe?g)$",
    re.IGNORECASE,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sync Reflection set 3 bilingual flashcard images"
    )
    ...
    report: list[str] = ["# Reflection set 3 bilingual flashcard image sync\n\n"]
    if all_skipped:
    ...
    for num, card_key in enumerate(CARD_ORDER, start=1):
        ...
        for lang_key, json_key in (("en", "en"), ("zh", "zhHant")):
            ...
            front_src = g["front"]
            back_src = g.get("back")
            front_rel = f"./flashcards/reflection/{prefix}-{lang_key}-front.webp"
            back_rel = f"./flashcards/reflection/{prefix}-{lang_key}-back.webp"
